...
g=Math.min(g,Math.ceil(f.y1));if(e.x0==f.x0&&e.y0==f.y0?e.x0+f.dy/e.dy*e.dx<f.x1:e.x1-f.dy/e.dy*e.dx<f.x0){var c=e;e=f;f=c}c=e.dx/e.dy;var d=f.dx/f.dy,l=e.dx>0,n=f.dx<0;for(a=a;a<g;a++){var q=c*Math.max(0,Math.min(e.dy,a+l-e.y0))+e.x0;h(Math.floor(d*Math.max(0,Math.min(f.dy,a+n-f.y0))+f.x0),Math.ceil(q),a)}}function da(e,f,a,g,h,c){var d=Z(e,f);f=Z(f,a);e=Z(a,e);if(d.dy>f.dy){a=d;d=f;f=a}if(d.dy>e.dy){a=d;d=e;e=a}if(f.dy>e.dy){a=f;f=e;e=a}d.dy&&ca(e,d,g,h,c);f.dy&&ca(e,f,g,h,c)}r.version="2.1+3.1+1";
