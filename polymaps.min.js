...
true;h.dispatch({type:"load",tile:f,features:H})}var n=f.element=s.svg("g");m=m(f);if(l)q({features:l});else f.request=e(j(f),q);t&&n.setAttribute("clip-path","url(#"+t+")")},function(f){f.request&&f.request.abort(true);delete o[f.key]}),j=s.url("about:blank"),k=true,t,o={},l;if(!arguments.length)e=s.queue.json;var r={Point:function(f,m){return g(f.coordinates,m)},MultiPoint:function(f,m){for(var q=s.svg("g"),n=0;n<f.coordinates.length;n++)q.appendChild(g(f.coordinates[n],m));return q},LineString:function(f,
m){return d(a,[f.coordinates],0,m)},MultiLineString:function(f,m){return d(a,f.coordinates,0,m)},Polygon:function(f,m){return d(c,[f.coordinates],1,m)},MultiPolygon:function(f,m){return d(c,f.coordinates||f.coords,1,m)},GeometryCollection:function(f,m){for(var q=s.svg("g"),n=0;n<f.geometries.length;n++){var w=f.geometries[n]&&f.geometries[n].type in r&&r[f.geometries[n].type](f.geometries[n],m);w&&q.appendChild(w)}return q}};h.url=function(f){if(!arguments.length)return j;j=typeof f=="string"?s.url(f):
f;return h};h.features=function(f){if(!arguments.length)return l;f&&h.tile(false);l=f;return h};h.clip=function(f){if(!arguments.length)return k;k=f;return h};h.init=function(f){if(k&&h.tile()){var m=h.map().tileSize(),q=f.insertBefore(s.svg("clipPath"),f.firstChild),n=q.appendChild(s.svg("rect"));q.setAttribute("id",t="org.polymaps."+s.id());n.setAttribute("width",m.x);n.setAttribute("height",m.y)}f.setAttribute("fill-rule","evenodd")};h.show=function(f){h.dispatch({type:"show",tile:f,features:o[f.key]||
[]})};h.reshow=function(){var f=h.cache.locks();for(var m in f)h.show(f[m])};return h};s.dblclick=function(){function e(d){var h=a.zoom();h=d.shiftKey?Math.ceil(h)-h-1:1-h+Math.floor(h);a.zoomBy(h,a.mouse(d))}var g={},a,c;g.map=function(d){if(!arguments.length)return a;if(a){c.removeEventListener("dblclick",e,false);c=null}if(a=d){c=a.container();c.addEventListener("dblclick",e,false)}return g};return g};s.drag=function(){function e(k){j={x:k.clientX,y:k.clientY};c.focusableParent().focus();k.preventDefault();
h=document.body.style.cursor;document.body.style.cursor="move"}function g(k){if(j){c.panBy({x:k.clientX-j.x,y:k.clientY-j.y});j.x=k.clientX;j.y=k.clientY}}var a={},c,d,h,j;a.map=function(k){if(!arguments.length)return c;if(c){d.removeEventListener("mousedown",e,false);d=null}if(c=k){d=c.container();d.addEventListener("mousedown",e,false)}return a};window.addEventListener("mousemove",g,false);window.addEventListener("mouseup",function(k){if(j){g(k);j=null;document.body.style.cursor=h}},false);return a};
s.wheel=function(){function e(){j=null}function g(o){var l=(o.wheelDelta/120||-o.detail)*0.1,r=k.mouse(o);if(Y<0){var f=Date.now(),m=f-d;if(m>9&&Math.abs(o.wheelDelta)/m>=50)Y=1;d=f}if(Y==1)l*=0.03;j||(j=k.pointLocation(r));k.off("move",e);if(h)k.zoomBy(l,r,j);else if(l){f=Date.now();if(f-c>200){k.zoomBy(l>0?+1:-1,r,j);c=f}}k.on("move",e);o.preventDefault();return false}var a={},c=0,d=0,h=true,j,k,t;a.smooth=function(o){if(!arguments.length)return h;h=o;return a};a.map=function(o){if(!arguments.length)return k;
if(k){t.removeEventListener("mousemove",e,false);t.removeEventListener("mousewheel",g,false);t.removeEventListener("DOMMouseScroll",g,false);t=null;k.off("move",e)}if(k=o){k.on("move",e);t=k.container();t.addEventListener("mousemove",e,false);t.addEventListener("mousewheel",g,false);t.addEventListener("DOMMouseScroll",g,false)}return a};return a};var Y=/WebKit\/533/.test(navigator.userAgent)?-1:0;s.arrow=function(){function e(f){if(!(f.ctrlKey||f.altKey||f.metaKey)){var m=Date.now(),q=0,n=0,w=0;switch(f.keyCode){case 37:if(!d.left){h=
m;d.left=1;d.right||(q=o)}break;case 39:if(!d.right){h=m;d.right=1;d.left||(q=-o)}break;case 38:if(!d.up){h=m;d.up=1;d.down||(n=o)}break;case 40:if(!d.down){h=m;d.down=1;d.up||(n=-o)}break;case 109:case 189:if(!d.plus){h=m;d.plus=1;d.minus||(w=-1)}break;case 61:case 187:if(!d.minus){h=m;d.minus=1;d.plus||(w=1)}break;default:return}if(w){m=l.zoom();l.zoom(w<0?Math.ceil(m)-1:Math.floor(m)+1)}else if(q||n)l.panBy({x:q,y:n});if(!j&&d.left|d.right|d.up|d.down)j=setInterval(a,t);f.preventDefault()}}function g(f){h=
Date.now();switch(f.keyCode){case 37:d.left=0;break;case 39:d.right=0;break;case 38:d.up=0;break;case 40:d.down=0;break;case 109:case 189:d.plus=0;break;case 61:case 187:d.minus=0;break;default:return}if(j&&!(d.left|d.right|d.up|d.down))j=clearInterval(j);f.preventDefault()}function a(){if(l)if(!(Date.now()<h+k)){var f=(d.left-d.right)*o,m=(d.up-d.down)*o;if(f||m)l.panBy({x:f,y:m})}}var c={},d={left:0,right:0,up:0,down:0,plus:0,minus:0},h=0,j,k=250,t=50,o=16,l,r;c.map=function(f){if(!arguments.length)return l;
if(l){r.removeEventListener("keydown",e,false);r.removeEventListener("keyup",g,false);r=null}if(l=f){r=l.focusableParent();r.addEventListener("keydown",e,false);r.addEventListener("keyup",g,false)}return c};c.speed=function(f){if(!arguments.length)return o;o=f;return c};return c};s.hash=function(){function e(){var j=h.center(),k=h.zoom(),t=Math.max(0,Math.ceil(Math.log(k)/Math.LN2));j="#"+k.toFixed(2)+"/"+j.lat.toFixed(t)+"/"+j.lon.toFixed(t);if(c!==j)location.replace(c=j)}function g(){if(location.hash!==
c){var j=(c=location.hash).substring(1).split("/").map(Number);if(j.length<3||j.some(isNaN))e();else{var k=h.size();h.zoomBy(j[0]-h.zoom(),{x:k.x/2,y:k.y/2},{lat:Math.min(d,Math.max(-d,j[1])),lon:j[2]})}}}var a={},c,d=89.99999999,h;a.map=function(j){if(!arguments.length)return h;if(h){h.off("move",e);window.removeEventListener("hashchange",g,false)}if(h=j){h.on("move",e);window.addEventListener("hashchange",g,false);location.hash?g():e()}return a};return a};s.interact=function(){var e={},g=s.drag(),
a=s.wheel(),c=s.dblclick(),d=s.arrow();e.map=function(h){g.map(h);a.map(h);c.map(h);d.map(h);return e};return e};s.compass=function(){function e(u){n.setAttribute("class","compass active");F||(F=setInterval(g,E));I&&D.panBy(I);L=Date.now();return t(u)}function g(){I&&Date.now()>L+C&&D.panBy(I)}function a(){n.setAttribute("class","compass");if(F){clearInterval(F);F=0}}function c(u){return function(){u?this.setAttribute("class","active"):this.removeAttribute("class");I=u}}function d(u){return function(z){n.setAttribute("class",
"compass active");var A=D.zoom();D.zoom(u<0?Math.ceil(A)-1:Math.floor(A)+1);return t(z)}}function h(u){return function(z){D.zoom(u);return t(z)}}function j(){this.setAttribute("class","active")}function k(){this.removeAttribute("class")}function t(u){u.stopPropagation();u.preventDefault();return false}function o(u){var z=Math.SQRT1_2*v,A=v*0.7,B=v*0.2,G=s.svg("g"),J=G.appendChild(s.svg("path")),K=G.appendChild(s.svg("path"));J.setAttribute("class","direction");J.setAttribute("pointer-events","all");
J.setAttribute("d","M0,0L"+z+","+z+"A"+v+","+v+" 0 0,1 "+-z+","+z+"Z");K.setAttribute("class","chevron");K.setAttribute("d","M"+B+","+(A-B)+"L0,"+A+" "+-B+","+(A-B));K.setAttribute("pointer-events","none");G.addEventListener("mousedown",e,false);G.addEventListener("mouseover",c(u),false);G.addEventListener("mouseout",c(null),false);G.addEventListener("dblclick",t,false);return G}function l(u){var z=v*0.4,A=z/2,B=s.svg("g"),G=B.appendChild(s.svg("path")),J=B.appendChild(s.svg("path")),K=B.appendChild(s.svg("path")),
P=B.appendChild(s.svg("path"));G.setAttribute("class","back");G.setAttribute("d","M"+-z+",0V"+-z+"A"+z+","+z+" 0 1,1 "+z+","+-z+"V0Z");J.setAttribute("class","direction");J.setAttribute("d",G.getAttribute("d"));K.setAttribute("class","chevron");K.setAttribute("d","M"+-A+","+-z+"H"+A+(u>0?"M0,"+(-z-A)+"V"+-A:""));P.setAttribute("class","fore");P.setAttribute("fill","none");P.setAttribute("d",G.getAttribute("d"));B.addEventListener("mousedown",d(u),false);B.addEventListener("mouseover",j,false);B.addEventListener("mouseout",
k,false);B.addEventListener("dblclick",t,false);return B}function r(u){var z=v*0.2,A=v*0.4,B=s.svg("g"),G=B.appendChild(s.svg("rect")),J=B.appendChild(s.svg("path"));G.setAttribute("pointer-events","all");G.setAttribute("fill","none");G.setAttribute("x",-A);G.setAttribute("y",-0.75*A);G.setAttribute("width",2*A);G.setAttribute("height",1.5*A);J.setAttribute("class","chevron");J.setAttribute("d","M"+-z+",0H"+z);B.addEventListener("mousedown",h(u),false);B.addEventListener("dblclick",t,false);return B}
function f(){var u=v+6,z=u,A=D.size();switch(i){case "top-left":break;case "top-right":u=A.x-u;break;case "bottom-left":z=A.y-z;break;case "bottom-right":u=A.x-u;z=A.y-z;break}n.setAttribute("transform","translate("+u+","+z+")");for(var B in w)B==D.zoom()?w[B].setAttribute("class","active"):w[B].removeAttribute("class")}function m(){for(;n.lastChild;)n.removeChild(n.lastChild);if(x!="none"){var u=n.appendChild(s.svg("g"));u.setAttribute("class","pan");var z=u.appendChild(s.svg("circle"));z.setAttribute("class",
"back");z.setAttribute("r",v);u.appendChild(o({x:0,y:-H})).setAttribute("transform","rotate(0)");u.appendChild(o({x:H,y:0})).setAttribute("transform","rotate(90)");u.appendChild(o({x:0,y:H})).setAttribute("transform","rotate(180)");u.appendChild(o({x:-H,y:0})).setAttribute("transform","rotate(270)");u=u.appendChild(s.svg("circle"));u.setAttribute("fill","none");u.setAttribute("class","fore");u.setAttribute("r",v)}if(y!="none"){u=n.appendChild(s.svg("g"));u.setAttribute("class","zoom");z=-0.5;if(y==
"big"){w={};var A=D.zoomRange()[0];for(z=0;A<=D.zoomRange()[1];A++,z++)(w[A]=u.appendChild(r(A))).setAttribute("transform","translate(0,"+-(z+0.75)*v*0.4+")")}A=x=="none"?0.4:2;u.setAttribute("transform","translate(0,"+v*(/^top-/.test(i)?A+(z+0.5)*0.4:-A)+")");u.appendChild(l(+1)).setAttribute("transform","translate(0,"+-(z+0.5)*v*0.4+")");u.appendChild(l(-1)).setAttribute("transform","scale(-1)")}f()}var q={},n=s.svg("g"),w={},v=30,H=16,L=0,C=250,E=50,i="top-left",y="small",x="small",F,I,D;n.setAttribute("class",
"compass");q.radius=function(u){if(!arguments.length)return v;v=u;D&&m();return q};q.speed=function(u){if(!arguments.length)return v;H=u;return q};q.position=function(u){if(!arguments.length)return i;i=u;D&&m();return q};q.pan=function(u){if(!arguments.length)return x;x=u;D&&m();return q};q.zoom=function(u){if(!arguments.length)return y;y=u;D&&m();return q};q.map=function(u){if(!arguments.length)return D;if(D){n.parentNode.removeChild(n);D.off("move",f).off("resize",f);window.removeEventListener("mouseup",
a,false)}if(D=u){window.addEventListener("mouseup",a,false);D.on("move",f).on("resize",f);D.container().appendChild(n);m()}return q};return q};s.grid=function(){function e(){var d=c.firstChild,h=a.size(),j=a.pointLocation(S);a.pointLocation(h);var k=Math.pow(2,4-Math.round(a.zoom()));j.lat=Math.floor(j.lat/k)*k;j.lon=Math.ceil(j.lon/k)*k;for(var t;(t=a.locationPoint(j).x)<=h.x;j.lon+=k){d||(d=c.appendChild(s.svg("line")));d.setAttribute("x1",t);d.setAttribute("x2",t);d.setAttribute("y1",0);d.setAttribute("y2",
h.y);d=d.nextSibling}for(;(t=a.locationPoint(j).y)<=h.y;j.lat-=k){d||(d=c.appendChild(s.svg("line")));d.setAttribute("y1",t);d.setAttribute("y2",t);d.setAttribute("x1",0);d.setAttribute("x2",h.x);d=d.nextSibling}for(;d;){h=d.nextSibling;c.removeChild(d);d=h}}var g={},a,c=s.svg("g");c.setAttribute("class","grid");g.map=function(d){if(!arguments.length)return a;if(a){c.parentNode.removeChild(c);a.off("move",e).off("resize",e)}if(a=d){a.on("move",e).on("resize",e);a.container().appendChild(c);a.dispatch({type:"move"})}return g};
