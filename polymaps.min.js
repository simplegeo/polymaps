...
i&&q.removeChild(y);if(i=l)q.insertBefore(y,q.firstChild);var g=k.cache.locks();for(var p in g)i?g[p].element.setAttribute("clip-path",u):g[p].element.removeAttribute("clip-path");return k};var D=k.tile;k.tile=function(l){arguments.length&&!l&&k.clip(l);return D.apply(k,arguments)};var x=k.map;k.map=function(l){if(l&&K){var g=l.tileSize();K.setAttribute("width",g.x);K.setAttribute("height",g.y)}return x.apply(k,arguments)};k.scale=function(l){if(!arguments.length)return w;(w=l)?k.on("zoom",o):k.off("zoom",
o);return k};k.show=function(l){i?l.element.setAttribute("clip-path",u):l.element.removeAttribute("clip-path");k.dispatch({type:"show",tile:l,features:l.features});return k};k.reshow=function(){var l=k.cache.locks();for(var g in l)k.show(l[g]);return k};return k};r.dblclick=function(){function d(c){var e=j.zoom();e=c.shiftKey?Math.ceil(e)-e-1:1-e+Math.floor(e);a==="mouse"?j.zoomBy(e,j.mouse(c)):j.zoomBy(e)}var f={},a="mouse",j,h;f.zoom=function(c){if(!arguments.length)return a;a=c;return f};f.map=
