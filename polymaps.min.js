...
"application/json")},image:function(g,k,t){function n(){l=document.createElement("img");l.onerror=function(){e--;d()};l.onload=function(){e--;t(l);d()};l.src=k;g.setAttributeNS(r.ns.xlink,"href",k)}var l;m.push(n);d();return{abort:function(i){if(h(n))return true;if(i&&l){l.src="about:";return true}return false}}}}}();r.map=function(){function d(){if(o)if(n<o[0])n=o[0];else if(n>o[1])n=o[1];l=n-(n=Math.round(n));i=Math.pow(2,l)}function h(){if(q){var f=45/Math.pow(2,n+l-3),u=Math.max(Math.abs(C*g.x+
A*g.y),Math.abs(B*g.x+E*g.y)),s=V(K-u*f/k.y);u=V(j+u*f/k.y);t.lat=Math.max(s,Math.min(u,t.lat));s=Math.max(Math.abs(C*g.y+A*g.x),Math.abs(B*g.y+E*g.x));t.lon=Math.max(q[0].lon-s*f/k.x,Math.min(q[1].lon+s*f/k.x,t.lon))}}var a={},m,e,c=Y,g=Y,k={x:256,y:256},t={lat:37.76487,lon:-122.41948},n=12,l=0,i=1,o=[1,18],w=0,A=1,C=0,E=1,B=0,K=-180,j=180,q=[{lat:V(K),lon:-Infinity},{lat:V(j),lon:Infinity}];a.locationCoordinate=function(f){f=r.map.locationCoordinate(f);var u=Math.pow(2,n);f.column*=u;f.row*=u;f.zoom+=
...
(B*s+E*f)/k.y)})};a.centerRange=function(f){if(!arguments.length)return q;if(q=f){K=q[0].lat>-90?W(q[0].lat):-Infinity;j=q[0].lat<90?W(q[1].lat):Infinity}else{K=-Infinity;j=Infinity}h();a.dispatch({type:"move"});return a};a.zoom=function(f){if(!arguments.length)return n+l;n=f;d();return a.center(t)};a.zoomBy=function(f,u,s){if(arguments.length<2)return a.zoom(n+l+f);if(arguments.length<3)s=a.pointLocation(u);n=n+l+f;d();var z=a.locationPoint(s);return a.panBy({x:u.x-z.x,y:u.y-z.y})};a.zoomRange=function(f){if(!arguments.length)return o;
o=f;return a.zoom(n+l)};a.extent=function(f){if(!arguments.length)return[a.pointLocation({x:0,y:c.y}),a.pointLocation({x:c.x,y:0})];var u=a.locationPoint(f[0]),s=a.locationPoint(f[1]),z=Math.max((s.x-u.x)/c.x,(u.y-s.y)/c.y);u=a.pointLocation({x:(u.x+s.x)/2,y:(u.y+s.y)/2});n=n+l-Math.log(z)/Math.LN2;d();return a.center(u)};a.angle=function(f){if(!arguments.length)return w;w=f;A=Math.cos(w);C=Math.sin(w);E=Math.cos(-w);B=Math.sin(-w);h();a.dispatch({type:"move"});return a};a.add=function(f){f.map(a);
...
g.load(J);P.proxyCount++;G.proxyRefs[P.key]=P}else M=false;if(M)break}if(!M)for(Q=1;Q<=D;Q++)if((P=g.peek(J={column:N>>Q,row:T>>Q,zoom:U-Q}))&&P.ready){O[P.key]=g.load(J);P.proxyCount++;G.proxyRefs[P.key]=P;break}}O[G.key]=G}}var j=c.map(),q=j.zoom(),x=q-(q=Math.round(q)),f=j.size(),u=j.angle(),s=j.tileSize(),z=j.locationCoordinate(j.center());if(A!=q){if(A<q)a(q);else if(A>q)m(q);else A=q;for(var v=-4;v<=2;v++){var L=C[v];L.setAttribute("class","zoom"+(v<0?"":"+")+v+" zoom"+(q+v));L.setAttribute("transform",
"scale("+Math.pow(2,-v)+")")}}o.setAttribute("transform","translate("+f.x/2+","+f.y/2+")"+(u?"rotate("+u/Math.PI*180+")":"")+(x?"scale("+Math.pow(2,x)+")":"")+(w?w.zoomFraction(x):""));var I=j.pointCoordinate(z,Y);v=j.pointCoordinate(z,{x:f.x,y:0});q=j.pointCoordinate(z,f);j=j.pointCoordinate(z,{x:0,y:f.y});if(!x&&!u&&!w){z.column=(Math.round(s.x*z.column)+(f.x&1)/2)/s.x;z.row=(Math.round(s.y*z.row)+(f.y&1)/2)/s.y}if(w){I=w.unapply(I);v=w.unapply(v);q=w.unapply(q);j=w.unapply(j);z=w.unapply(z)}var S=
n?n(I.zoom)-I.zoom:0;if(S){f=Math.pow(2,S);I.column*=f;I.row*=f;v.column*=f;v.row*=f;q.column*=f;q.row*=f;j.column*=f;j.row*=f;I.zoom=v.zoom=q.zoom=j.zoom+=S}x=g.locks();var O={};for(var F in x)x[F].proxyCount=0;if(t&&S>-5&&S<3){u=I.zoom<0?1:1<<I.zoom;if(k){ca(I,v,q,0,u,K);ca(q,j,I,0,u,K)}else{f=Math.floor((I.column+q.column)/2);u=Math.max(0,Math.min(u-1,Math.floor((v.row+j.row)/2)));v=Math.min(4,I.zoom);f=f>>v<<v;u=u>>v<<v;K(f,f+1,u)}}for(F in O){v=O[F];f=Math.pow(2,v.level=v.zoom-z.zoom);v.element.setAttribute("transform",
"translate("+(v.x=s.x*(v.column-z.column*f))+","+(v.y=s.y*(v.row-z.row*f))+")")}for(F in x)if(!(F in O)){v=g.unload(F);v.element.parentNode.removeChild(v.element);delete v.proxyRefs}for(F in O){v=O[F];if(v.element.parentNode!=C[v.level]){C[v.level].appendChild(v.element);c.show&&c.show(v)}}g.flush();c.dispatch({type:"move"})}var c={},g=c.cache=r.cache(d,h).size(512),k=true,t=true,n,l,i,o=r.svg("g"),w,A,C={};o.setAttribute("class","layer");for(var E=-4;E<=-1;E++)C[E]=o.appendChild(r.svg("g"));for(E=
2;E>=1;E--)C[E]=o.appendChild(r.svg("g"));C[0]=o.appendChild(r.svg("g"));c.map=function(B){if(!arguments.length)return i;if(i){if(i==B){o.parentNode.appendChild(o);return c}i.off("move",e).off("resize",e);o.parentNode.removeChild(o)}if(i=B){i.container().appendChild(o);c.init&&c.init(o);i.on("move",e).on("resize",e);e()}return c};c.container=function(){return o};c.levels=function(){return C};c.id=function(B){if(!arguments.length)return l;l=B;o.setAttribute("id",B);return c};c.visible=function(B){if(!arguments.length)return t;
(t=B)?o.removeAttribute("visibility"):o.setAttribute("visibility","hidden");i&&e();return c};c.transform=function(B){if(!arguments.length)return w;w=B;i&&e();return c};c.zoom=function(B){if(!arguments.length)return n;n=typeof B=="function"||B==null?B:function(){return B};i&&e();return c};c.tile=function(B){if(!arguments.length)return k;k=B;i&&e();return c};c.reload=function(){g.clear();i&&e();return c};c.dispatch=r.dispatch(c);c.on("load",function(B){if(B.tile.proxyRefs){for(var K in B.tile.proxyRefs){var j=
...
d.dispatch({type:"load",tile:a})});else{a.ready=true;d.dispatch({type:"load",tile:a})}}else{a.ready=true;h!=null&&m.setAttributeNS(r.ns.xlink,"href",h);d.dispatch({type:"load",tile:a})}},function(a){a.request&&a.request.abort(true)}),h;d.url=function(a){if(!arguments.length)return h;h=typeof a=="string"&&/{.}/.test(a)?r.url(a):a;return d.reload()};return d};r.geoJson=function(d){function h(j){var q={lat:0,lon:0};return function(x){q.lat=x[1];q.lon=x[0];var f=j(q);x.x=f.x;x.y=f.y;return f}}function a(j,
q){return j&&j.type in C&&C[j.type](j,q)}function m(j,q,x){return j.type in E&&E[j.type](j,q,x)}function e(){var j=c.map().zoom(),q=c.cache.locks(),x,f,u,s,z,v,L;if(w=="fixed")for(x in q){if((f=q[x]).scale!=j){L="scale("+Math.pow(2,f.zoom-j)+")";s=-1;for(z=(u=f.features).length;++s<z;)m((v=u[s]).data.geometry,v.element,L);f.scale=j}}else for(x in q){s=-1;for(z=(u=(f=q[x]).features).length;++s<z;)m((v=u[s]).data.geometry,v.element,"");delete f.scale}}var c=r.layer(function(j,q){function x(u){var s=
[];if(u.next)j.request=d(u.next.href,x);switch(u.type){case "FeatureCollection":for(var z=0;z<u.features.length;z++){var v=u.features[z],L=a(v.geometry,q);L&&s.push({element:f.appendChild(L),data:v})}break;case "Feature":(L=a(u.geometry,q))&&s.push({element:f.appendChild(L),data:u});break;default:(L=a(u,q))&&s.push({element:f.appendChild(L),data:{type:"Feature",geometry:u}});break}j.ready=true;s.push.apply(j.features,s);c.dispatch({type:"load",tile:j,features:s})}var f=j.element=r.svg("g");j.features=
[];q=h(q(j).locationPoint);if(k!=null)j.request=d(typeof k=="function"?k(j):k,x);else x({type:"FeatureCollection",features:A||[]})},function(j){j.request&&j.request.abort(true)}),g=c.container(),k,t=true,n="org.polymaps."+r.id(),l="url(#"+n+")",i=g.insertBefore(r.svg("clipPath"),g.firstChild),o=i.appendChild(r.svg("rect")),w="auto",A;g.setAttribute("fill-rule","evenodd");i.setAttribute("id",n);if(!arguments.length)d=r.queue.json;var C={Point:function(j,q){var x=q(j.coordinates),f=r.svg("circle");
f.setAttribute("r",4.5);f.setAttribute("transform","translate("+x.x+","+x.y+")");return f},MultiPoint:function(j,q){for(var x=r.svg("g"),f=j.coordinates,u,s,z=-1,v=f.length;++z<v;){s=x.appendChild(r.svg("circle"));s.setAttribute("r",4.5);s.setAttribute("transform","translate("+(u=q(f[z])).x+","+u.y+")")}return x},LineString:function(j,q){for(var x=r.svg("path"),f=["M"],u=j.coordinates,s,z=-1,v=u.length;++z<v;)f.push((s=q(u[z])).x,",",s.y,"L");f.pop();if(f.length){x.setAttribute("d",f.join(""));return x}},
MultiLineString:function(j,q){for(var x=r.svg("path"),f=[],u=j.coordinates,s,z=-1,v,L=u.length,I;++z<L;){s=u[z];v=-1;I=s.length;for(f.push("M");++v<I;)f.push((p=q(s[v])).x,",",p.y,"L");f.pop()}if(f.length){x.setAttribute("d",f.join(""));return x}},Polygon:function(j,q){for(var x=r.svg("path"),f=[],u=j.coordinates,s,z=-1,v,L=u.length,I;++z<L;){s=u[z];v=-1;I=s.length-1;for(f.push("M");++v<I;)f.push((p=q(s[v])).x,",",p.y,"L");f[f.length-1]="Z"}if(f.length){x.setAttribute("d",f.join(""));return x}},MultiPolygon:function(j,
q){for(var x=r.svg("path"),f=[],u=j.coordinates,s,z,v=-1,L,I,S=u.length,O,F;++v<S;){s=u[v];L=-1;for(O=s.length;++L<O;){z=s[L];I=-1;F=z.length-1;for(f.push("M");++I<F;)f.push((p=q(z[I])).x,",",p.y,"L");f[f.length-1]="Z"}}if(f.length){x.setAttribute("d",f.join(""));return x}},GeometryCollection:function(j,q){for(var x=r.svg("g"),f=-1,u=j.geometries,s=u.length,z;++f<s;)(z=a(u[f],q))&&x.appendChild(z);return x}},E={Point:function(j,q,x){j=j.coordinates;q.setAttribute("transform","translate("+j.x+","+
j.y+")"+x)},MultiPoint:function(j,q,x){j=j.coordinates;var f=-1,u=s.length;q=q.firstChild;for(var s;++f<u;){s=j[f];q.setAttribute("transform","translate("+s.x+","+s.y+")"+x);q=q.nextSibling}}};c.url=function(j){if(!arguments.length)return k;k=typeof j=="string"&&/{.}/.test(j)?r.url(j):j;if(k!=null)A=null;typeof k=="string"&&c.tile(false);return c.reload()};c.features=function(j){if(!arguments.length)return A;if(A=j){k=null;c.tile(false)}return c.reload()};c.clip=function(j){if(!arguments.length)return t;
t&&g.removeChild(i);if(t=j)g.insertBefore(i,g.firstChild);var q=c.cache.locks();for(var x in q)t?q[x].element.setAttribute("clip-path",l):q[x].element.removeAttribute("clip-path");return c};var B=c.tile;c.tile=function(j){arguments.length&&!j&&c.clip(j);return B.apply(c,arguments)};var K=c.map;c.map=function(j){if(j&&o){var q=j.tileSize();o.setAttribute("width",q.x);o.setAttribute("height",q.y)}return K.apply(c,arguments)};c.scale=function(j){if(!arguments.length)return w;(w=j)?c.on("move",e):c.off("move",
e);c.map()&&e();return c};c.show=function(j){t?j.element.setAttribute("clip-path",l):j.element.removeAttribute("clip-path");c.dispatch({type:"show",tile:j,features:j.features});return c};c.reshow=function(){var j=c.cache.locks();for(var q in j)c.show(j[q]);return c};return c};r.dblclick=function(){function d(c){var g=m.zoom();g=c.shiftKey?Math.ceil(g)-g-1:1-g+Math.floor(g);a==="mouse"?m.zoomBy(g,m.mouse(c)):m.zoomBy(g)}var h={},a="mouse",m,e;h.zoom=function(c){if(!arguments.length)return a;a=c;return h};
...
(c.left-c.right)*l,A=(c.up-c.down)*l;if(w||A)i.panBy({x:w,y:A})}}var e={},c={left:0,right:0,up:0,down:0},g=0,k,t=250,n=50,l=16,i,o;e.map=function(w){if(!arguments.length)return i;if(i){o.removeEventListener("keypress",a,false);o.removeEventListener("keydown",d,false);o.removeEventListener("keyup",h,false);o=null}if(i=w){o=i.focusableParent();o.addEventListener("keypress",a,false);o.addEventListener("keydown",d,false);o.addEventListener("keyup",h,false)}return e};e.speed=function(w){if(!arguments.length)return l;
...
k.toFixed(2)+"/"+t.lat.toFixed(n)+"/"+t.lon.toFixed(n)};a.map=function(k){if(!arguments.length)return e;if(e){e.off("move",d);window.removeEventListener("hashchange",h,false)}if(e=k){e.on("move",d);window.addEventListener("hashchange",h,false);location.hash?h():d()}return a};a.parser=function(k){if(!arguments.length)return c;c=k;return a};a.formatter=function(k){if(!arguments.length)return g;g=k;return a};return a};r.touch=function(){function d(n){var l=-1,i=n.touches.length,o=Date.now();if(i==1&&
o-c<300){var w=a.zoom();a.zoomBy(1-w+Math.floor(w),a.mouse(n.touches[0]));n.preventDefault()}c=o;g=a.zoom();for(k=a.angle();++l<i;){o=n.touches[l];t[o.identifier]=a.pointLocation(a.mouse(o))}}var h={},a,m,e=false,c=0,g,k,t={};window.addEventListener("touchmove",function(n){switch(n.touches.length){case 1:var l=n.touches[0];a.zoomBy(0,a.mouse(l),t[l.identifier]);n.preventDefault();break;case 2:l=n.touches[0];var i=n.touches[1],o=a.mouse(l),w=a.mouse(i);o={x:(o.x+w.x)/2,y:(o.y+w.y)/2};l=r.map.locationCoordinate(t[l.identifier]);
i=r.map.locationCoordinate(t[i.identifier]);i=r.map.coordinateLocation({row:(l.row+i.row)/2,column:(l.column+i.column)/2,zoom:0});a.zoomBy(Math.log(n.scale)/Math.LN2+g-a.zoom(),o,i);e&&a.angle(n.rotation/180*Math.PI+k);n.preventDefault();break}},false);h.rotate=function(n){if(!arguments.length)return e;e=n;return h};h.map=function(n){if(!arguments.length)return a;if(a){m.removeEventListener("touchstart",d,false);m=null}if(a=n){m=a.container();m.addEventListener("touchstart",d,false)}return h};return h};
...
function C(){for(;B.lastChild;)B.removeChild(B.lastChild);B.appendChild(N);if(L!="none"){O=B.appendChild(r.svg("g"));O.setAttribute("class","pan");var y=O.appendChild(r.svg("circle"));y.setAttribute("class","back");y.setAttribute("r",j);O.appendChild(i({x:0,y:-q})).setAttribute("transform","rotate(0)");O.appendChild(i({x:q,y:0})).setAttribute("transform","rotate(90)");O.appendChild(i({x:0,y:q})).setAttribute("transform","rotate(180)");O.appendChild(i({x:-q,y:0})).setAttribute("transform","rotate(270)");
y=O.appendChild(r.svg("circle"));y.setAttribute("fill","none");y.setAttribute("class","fore");y.setAttribute("r",j)}else O=null;if(z!="none"){v=B.appendChild(r.svg("g"));v.setAttribute("class","zoom");y=-0.5;if(z=="big"){K={};var D=H.zoomRange()[0];for(y=0;D<=H.zoomRange()[1];D++,y++)(K[D]=v.appendChild(w(D))).setAttribute("transform","translate(0,"+-(y+0.75)*j*0.4+")")}D=L=="none"?0.4:2;v.setAttribute("transform","translate(0,"+j*(/^top-/.test(s)?D+(y+0.5)*0.4:-D)+")");v.appendChild(o(+1)).setAttribute("transform",
"translate(0,"+-(y+0.5)*j*0.4+")");v.appendChild(o(-1)).setAttribute("transform","scale(-1)")}else v=null;A()}var E={},B=r.svg("g"),K={},j=30,q=16,x=0,f=250,u=50,s="top-left",z="small",v,L="small",I,S,O,F,N=r.svg("rect"),H,T,U;B.setAttribute("class","compass");N.setAttribute("class","back fore");N.setAttribute("pointer-events","none");N.setAttribute("display","none");E.radius=function(y){if(!arguments.length)return j;j=y;H&&C();return E};E.speed=function(y){if(!arguments.length)return j;q=y;return E};
...
a.dispatch({type:"move"})}return h};return h};r.stylist=function(){function d(e){var c=e.features.length,g=h.length,k=a.length,t,n,l,i,o,w;for(o=0;o<c;++o)if(n=(t=e.features[o]).element){t=t.data;for(w=0;w<g;++w){i=(l=h[w]).value;if(typeof i==="function")i=i.call(null,t);i==null?l.name.local?n.removeAttributeNS(l.name.space,l.name.local):n.removeAttribute(l.name):l.name.local?n.setAttributeNS(l.name.space,l.name.local,i):n.setAttribute(l.name,i)}for(w=0;w<k;++w){i=(l=a[w]).value;if(typeof i==="function")i=
i.call(null,t);i==null?n.style.removeProperty(l.name):n.style.setProperty(l.name,i,l.priority)}if(i=m){if(typeof i==="function")i=i.call(null,t);if(n.lastChild&&n.lastChild.nodeName!="path"&&n.lastChild.nodeName!="circle")for(;n.lastChild;)n.removeChild(n.lastChild);if(i!=null)if(n.children.length)for(o=0;o<n.children.length;o++)n.children[o].appendChild(r.svg("title")).appendChild(document.createTextNode(i));else n.appendChild(r.svg("title")).appendChild(document.createTextNode(i))}}}var h=[],a=
[],m;d.attr=function(e,c){h.push({name:ea(e),value:c});return d};d.style=function(e,c,g){a.push({name:e,value:c,priority:arguments.length<3?null:g});return d};d.title=function(e){m=e;return d};return d}})(org.polymaps);
