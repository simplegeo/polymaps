...
e.dy*e.dx<f.x0){var c=e;e=f;f=c}c=e.dx/e.dy;var d=f.dx/f.dy,m=e.dx>0,n=f.dx<0;for(a=a;a<g;a++){var s=c*Math.max(0,Math.min(e.dy,a+m-e.y0))+e.x0;i(Math.floor(d*Math.max(0,Math.min(f.dy,a+n-f.y0))+f.x0),Math.ceil(s),a)}}function da(e,f,a,g,i,c){var d=Z(e,f);f=Z(f,a);e=Z(a,e);if(d.dy>f.dy){a=d;d=f;f=a}if(d.dy>e.dy){a=d;d=e;e=a}if(f.dy>e.dy){a=f;f=e;e=a}d.dy&&ca(e,d,g,i,c);f.dy&&ca(e,f,g,i,c)}o.version="2.0+2.3+6";var Y={x:0,y:0};o.id=function(){var e=0;return function(){return++e}}();o.svg=function(e){return document.createElementNS(o.ns.svg,
...
360;return{column:(e.lon+180)*f,row:(180-V(e.lat))*f,zoom:0}};o.map.coordinateLocation=function(e){var f=45/Math.pow(2,e.zoom-3);return{lon:f*e.column-180,lat:U(180-f*e.row)}};var $=/WebKit/.test(navigator.userAgent)?-1:0;o.layer=function(e,f){function a(h){for(var w=C[0].nextSibling;x<h;x++){u.insertBefore(C[-4],w);u.insertBefore(C[2],w);u.insertBefore(C[1],w);for(var t=C[-4],q=-4;q<2;)C[q]=C[++q];C[q]=t}}function g(h){for(var w=C[0].nextSibling;x>h;x--){u.insertBefore(C[-1],w);u.insertBefore(C[2],
C[-4]);for(var t=C[2],q=2;q>-4;)C[q]=C[--q];C[q]=t}}function i(){function h(L){var H=L.zoom,R=H<0?1:1<<H,S=L.column%R,v=L.row;if(S<0)S+=R;return{locationPoint:function(D){D=o.map.locationCoordinate(D);var G=Math.pow(2,H-D.zoom);return{x:z.x*(G*D.column-S),y:z.y*(G*D.row-v)}}}}function w(L,H,R){var S=O.zoom,v=2-T,D=4+T;for(L=L;L<H;L++){var G=d.load({column:L,row:R,zoom:S},h);if(!G.ready&&!(G.key in N)){G.proxyRefs={};for(var J,K,M,P=1;P<=v;P++){K=true;for(var W=0,ea=1<<P;W<=ea;W++)for(var aa=0;aa<=
ea;aa++)if((M=d.peek(J={column:(L<<P)+aa,row:(R<<P)+W,zoom:S+P}))&&M.ready){N[M.key]=d.load(J);M.proxyCount++;G.proxyRefs[M.key]=M}else K=false;if(K)break}if(!K)for(P=1;P<=D;P++)if((M=d.peek(J={column:L>>P,row:R>>P,zoom:S-P}))&&M.ready){N[M.key]=d.load(J);M.proxyCount++;G.proxyRefs[M.key]=M;break}}N[G.key]=G}}var t=c.map(),q=t.zoom(),B=q-(q=Math.round(q)),j=t.size(),r=t.angle(),z=t.tileSize(),E=t.locationCoordinate(t.center());if(x!=q){if(x<q)a(q);else if(x>q)g(q);else x=q;for(var A=-4;A<=2;A++){var X=
C[A];X.setAttribute("class","zoom"+(A<0?"":"+")+A+" zoom"+(q+A));X.setAttribute("transform","scale("+Math.pow(2,-A)+")")}}u.setAttribute("transform","translate("+j.x/2+","+j.y/2+")"+(r?"rotate("+r/Math.PI*180+")":"")+(B?"scale("+Math.pow(2,B)+")":"")+(y?y.zoomFraction(B):""));var O=t.pointCoordinate(E,Y);A=t.pointCoordinate(E,{x:j.x,y:0});q=t.pointCoordinate(E,j);t=t.pointCoordinate(E,{x:0,y:j.y});if(!B&&!r&&!y){E.column=(Math.round(z.x*E.column)+(j.x&1)/2)/z.x;E.row=(Math.round(z.y*E.row)+(j.y&1)/
2)/z.y}if(y){O=y.unapply(O);A=y.unapply(A);q=y.unapply(q);t=y.unapply(t);E=y.unapply(E)}var T=s?s(O.zoom)-O.zoom:0;if(T){r=Math.pow(2,T);O.column*=r;O.row*=r;A.column*=r;A.row*=r;q.column*=r;q.row*=r;t.column*=r;t.row*=r;O.zoom=A.zoom=q.zoom=t.zoom+=T}B=d.locks();var N={};for(var I in B)B[I].proxyCount=0;if(n&&T>-5&&T<3){r=O.zoom<0?1:1<<O.zoom;if(m){da(O,A,q,0,r,w);da(q,t,O,0,r,w)}else{j=Math.floor((O.column+q.column)/2);r=Math.max(0,Math.min(r-1,Math.floor((A.row+t.row)/2)));A=Math.min(4,O.zoom);
j=j>>A<<A;r=r>>A<<A;w(j,j+1,r)}}for(I in N){A=N[I];r=Math.pow(2,A.level=A.zoom-E.zoom);j=z.x*(A.column-E.column*r);r=z.y*(A.row-E.row*r);A.element.setAttribute("transform","translate("+j+","+r+")")}for(I in B)if(!(I in N)){A=d.unload(I);A.element.parentNode.removeChild(A.element);delete A.proxyRefs}for(I in N){A=N[I];if(A.element.parentNode!=C[A.level]){C[A.level].appendChild(A.element);c.show&&c.show(A)}}}var c={},d=c.cache=o.cache(e,f).size(512),m=true,n=true,s,l,k,u=o.svg("g"),y,x,C={};u.setAttribute("class",
"layer");for(var F=-4;F<=-1;F++)C[F]=u.appendChild(o.svg("g"));for(F=2;F>=1;F--)C[F]=u.appendChild(o.svg("g"));C[0]=u.appendChild(o.svg("g"));c.map=function(h){if(!arguments.length)return k;if(k){if(k==h){u.parentNode.appendChild(u);return c}k.off("move",i).off("resize",i);u.parentNode.removeChild(u)}if(k=h){k.container().appendChild(u);c.init&&c.init(u);k.on("move",i).on("resize",i);i()}return c};c.container=function(){return u};c.id=function(h){if(!arguments.length)return l;l=h;u.setAttribute("id",
h);return c};c.visible=function(h){if(!arguments.length)return n;n=h?u.removeAttribute("visibility"):u.setAttribute("visibility","hidden");k&&i();return c};c.transform=function(h){if(!arguments.length)return y;y=h;k&&i();return c};c.zoom=function(h){if(!arguments.length)return s;s=typeof h=="function"||h==null?h:function(){return h};return c};c.tile=function(h){if(!arguments.length)return m;m=h;return c};c.dispatch=o.dispatch(c);c.on("load",function(h){if(h.tile.proxyRefs){for(var w in h.tile.proxyRefs){var t=
...
else h.request=e(typeof n=="function"?n(h):n,t);l&&q.setAttribute("clip-path","url(#"+l+")")},function(h){h.request&&h.request.abort(true);delete y[h.key]}),m=d.container(),n="about:blank",s=true,l,k,u,y={},x;m.setAttribute("fill-rule","evenodd");if(!arguments.length)e=o.queue.json;var C={Point:function(h,w){return a(h.coordinates,w)},MultiPoint:function(h,w){for(var t=o.svg("g"),q=0;q<h.coordinates.length;q++)t.appendChild(a(h.coordinates[q],w));return t},LineString:function(h,w){return c(g,[h.coordinates],
...
h&&d.tile(false);x=h;return d};d.clip=function(h){if(!arguments.length)return s;s=h;return d};var F=d.map;d.map=function(h){if(h)if(s&&d.tile()){if(!k){k=m.insertBefore(o.svg("clipPath"),m.firstChild);u=k.appendChild(o.svg("rect"));k.setAttribute("id",l="org.polymaps."+o.id())}var w=h.tileSize();u.setAttribute("width",w.x);u.setAttribute("height",w.y)}else if(k){m.removeChild(k);k=u=l=null}return F.apply(d,arguments)};d.show=function(h){d.dispatch({type:"show",tile:h,features:y[h.key]||[]})};d.reshow=
...
I.x1.x),y:Math.min(I.x0.y,I.x1.y)})]);L.setAttribute("display","none")}I=null}if(O){clearInterval(O);O=0}}function c(v){return function(){v?this.setAttribute("class","active"):this.removeAttribute("class");T=v}}function d(v){return function(D){h.setAttribute("class","compass active");var G=H.zoom();H.zoom(v<0?Math.ceil(G)-1:Math.floor(G)+1);return l(D)}}function m(v){return function(D){H.zoom(v);return l(D)}}function n(){this.setAttribute("class","active")}function s(){this.removeAttribute("class")}
...
"chevron");P.setAttribute("d","M"+-G+","+-D+"H"+G+(v>0?"M0,"+(-D-G)+"V"+-G:""));W.setAttribute("class","fore");W.setAttribute("fill","none");W.setAttribute("d",K.getAttribute("d"));J.addEventListener("mousedown",d(v),false);J.addEventListener("mouseover",n,false);J.addEventListener("mouseout",s,false);J.addEventListener("dblclick",l,false);return J}function y(v){var D=t*0.2,G=t*0.4,J=o.svg("g"),K=J.appendChild(o.svg("rect")),M=J.appendChild(o.svg("path"));K.setAttribute("pointer-events","all");K.setAttribute("fill",
"none");K.setAttribute("x",-G);K.setAttribute("y",-0.75*G);K.setAttribute("width",2*G);K.setAttribute("height",1.5*G);M.setAttribute("class","chevron");M.setAttribute("d","M"+-D+",0H"+D);J.addEventListener("mousedown",m(v),false);J.addEventListener("dblclick",l,false);return J}function x(){var v=t+6,D=v,G=H.size();switch(z){case "top-left":break;case "top-right":v=G.x-v;break;case "bottom-left":D=G.y-D;break;case "bottom-right":v=G.x-v;D=G.y-D;break}v="translate("+v+","+D+")";N&&N.setAttribute("transform",
v);A&&A.setAttribute("transform",v);for(var J in w)J==H.zoom()?w[J].setAttribute("class","active"):w[J].removeAttribute("class")}function C(){for(;h.lastChild;)h.removeChild(h.lastChild);h.appendChild(L);if(X!="none"){N=h.appendChild(o.svg("g"));N.setAttribute("class","pan");var v=N.appendChild(o.svg("circle"));v.setAttribute("class","back");v.setAttribute("r",t);N.appendChild(k({x:0,y:-q})).setAttribute("transform","rotate(0)");N.appendChild(k({x:q,y:0})).setAttribute("transform","rotate(90)");N.appendChild(k({x:0,
y:q})).setAttribute("transform","rotate(180)");N.appendChild(k({x:-q,y:0})).setAttribute("transform","rotate(270)");v=N.appendChild(o.svg("circle"));v.setAttribute("fill","none");v.setAttribute("class","fore");v.setAttribute("r",t)}else N=null;if(E!="none"){A=h.appendChild(o.svg("g"));A.setAttribute("class","zoom");v=-0.5;if(E=="big"){w={};var D=H.zoomRange()[0];for(v=0;D<=H.zoomRange()[1];D++,v++)(w[D]=A.appendChild(y(D))).setAttribute("transform","translate(0,"+-(v+0.75)*t*0.4+")")}D=X=="none"?
0.4:2;A.setAttribute("transform","translate(0,"+t*(/^top-/.test(z)?D+(v+0.5)*0.4:-D)+")");A.appendChild(u(+1)).setAttribute("transform","translate(0,"+-(v+0.5)*t*0.4+")");A.appendChild(u(-1)).setAttribute("transform","scale(-1)")}else A=null;x()}var F={},h=o.svg("g"),w={},t=30,q=16,B=0,j=250,r=50,z="top-left",E="small",A,X="small",O,T,N,I,L=o.svg("rect"),H,R,S;h.setAttribute("class","compass");L.setAttribute("class","back fore");L.setAttribute("pointer-events","none");L.setAttribute("display","none");
F.radius=function(v){if(!arguments.length)return t;t=v;H&&C();return F};F.speed=function(v){if(!arguments.length)return t;q=v;return F};F.position=function(v){if(!arguments.length)return z;z=v;H&&C();return F};F.pan=function(v){if(!arguments.length)return X;X=v;H&&C();return F};F.zoom=function(v){if(!arguments.length)return E;E=v;H&&C();return F};F.map=function(v){if(!arguments.length)return H;if(H){R.removeEventListener("mousedown",a,false);R.removeChild(h);R=null;S.removeEventListener("mousemove",
g,false);S.removeEventListener("mouseup",i,false);S=null;H.off("move",x).off("resize",x)}if(H=v){R=H.container();R.appendChild(h);R.addEventListener("mousedown",a,false);S=R.ownerDocument.defaultView;S.addEventListener("mousemove",g,false);S.addEventListener("mouseup",i,false);H.on("move",x).on("resize",x);C()}return F};return F};o.grid=function(){function e(){var i=g.firstChild,c=a.size(),d=a.pointLocation(Y);a.pointLocation(c);var m=Math.pow(2,4-Math.round(a.zoom()));d.lat=Math.floor(d.lat/m)*m;
