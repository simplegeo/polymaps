...
k=Math.min(k,Math.ceil(f.y1));if(e.x0==f.x0&&e.y0==f.y0?e.x0+f.dy/e.dy*e.dx<f.x1:e.x1-f.dy/e.dy*e.dx<f.x0){var a=e;e=f;f=a}a=e.dx/e.dy;var g=f.dx/f.dy,n=e.dx>0,l=f.dx<0;for(c=c;c<k;c++){var s=a*Math.max(0,Math.min(e.dy,c+n-e.y0))+e.x0;i(Math.floor(g*Math.max(0,Math.min(f.dy,c+l-f.y0))+f.x0),Math.ceil(s),c)}}function da(e,f,c,k,i,a){var g=Z(e,f);f=Z(f,c);e=Z(c,e);if(g.dy>f.dy){c=g;g=f;f=c}if(g.dy>e.dy){c=g;g=e;e=c}if(f.dy>e.dy){c=f;f=e;e=c}g.dy&&ca(e,g,k,i,a);f.dy&&ca(e,f,k,i,a)}t.version="2.3.0";
var Y={x:0,y:0};t.ns={svg:"http://www.w3.org/2000/svg",xlink:"http://www.w3.org/1999/xlink"};t.id=function(){var e=0;return function(){return++e}}();t.svg=function(e){return document.createElementNS(t.ns.svg,e)};t.transform=function(e,f,c,k,i,a){var g={},n,l,s;if(!arguments.length){e=1;f=i=c=0;k=1;a=0}g.zoomFraction=function(m){if(!arguments.length)return l;l=m;n=Math.floor(l+Math.log(Math.sqrt(e*e+f*f+c*c+k*k))/Math.log(2));s=Math.pow(2,-n);return g};g.apply=function(m){var j=Math.pow(2,-m.zoom),
u=Math.pow(2,m.zoom-n);return{column:(e*m.column*j+c*m.row*j+i)*u,row:(f*m.column*j+k*m.row*j+a)*u,zoom:m.zoom-n}};g.unapply=function(m){var j=Math.pow(2,-m.zoom),u=Math.pow(2,m.zoom+n);return{column:(m.column*j*k-m.row*j*c-i*k+a*c)/(e*k-f*c)*u,row:(m.column*j*f-m.row*j*e-i*f+a*e)/(c*f-k*e)*u,zoom:m.zoom+n}};g.toString=function(){return"matrix("+[e*s,f*s,c*s,k*s].join(" ")+" 0 0)"};return g.zoomFraction(0)};t.cache=function(e,f){function c(j){m--;f&&f(j);delete g[j.key];if(j.next)j.next.prev=j.prev;
else if(l=j.prev)l.next=null;if(j.prev)j.prev.next=j.next;else if(n=j.next)n.prev=null}function k(){for(var j=l;m>s;j=j.prev){if(!j)break;j.lock||c(j)}}var i={},a={},g={},n=null,l=null,s=64,m=0;i.peek=function(j){return g[[j.zoom,j.column,j.row].join("/")]};i.load=function(j,u){var y=[j.zoom,j.column,j.row].join("/"),A=g[y];if(A){if(A.prev){if(A.prev.next=A.next)A.next.prev=A.prev;else l=A.prev;A.prev=null;A.next=n;n=n.prev=A}A.lock=1;return a[y]=A}A={key:y,column:j.column,row:j.row,zoom:j.zoom,next:n,
prev:null,lock:1};e.call(null,A,u);a[y]=g[y]=A;if(n)n.prev=A;else l=A;n=A;m++;return A};i.unload=function(j){if(!(j in a))return false;var u=a[j];u.lock=0;delete a[j];u.request&&u.request.abort(false)&&c(u);return u};i.locks=function(){return a};i.size=function(j){if(!arguments.length)return s;s=j;k();return i};i.flush=function(){k();return i};i.clear=function(){for(var j in g){var u=g[j];u.request&&u.request.abort(false);f&&f(g[j]);if(u.lock){u.lock=0;u.element.parentNode.removeChild(u.element)}}a=
{};g={};n=l=null;m=0;return i};return i};t.url=function(e){function f(k){var i=k.zoom<0?1:1<<k.zoom,a=k.column%i;if(a<0)a+=i;return e.replace(/{(.)}/g,function(g,n){switch(n){case "S":return c[(Math.abs(k.zoom)+k.row+a)%c.length];case "Z":return k.zoom;case "X":return a;case "Y":return k.row;case "B":var l=t.map.coordinateLocation({row:k.row,column:a,zoom:k.zoom}),s=t.map.coordinateLocation({row:k.row+1,column:a+1,zoom:k.zoom}),m=Math.ceil(Math.log(k.zoom)/Math.LN2);return s.lat.toFixed(m)+","+l.lon.toFixed(m)+
","+l.lat.toFixed(m)+","+s.lon.toFixed(m)}return n})}var c=[];f.template=function(k){if(!arguments.length)return e;e=k;return f};f.hosts=function(k){if(!arguments.length)return c;c=k;return f};return f};t.dispatch=function(e){var f={};e.on=function(c,k){for(var i=f[c]||(f[c]=[]),a=0;a<i.length;a++)if(i[a].handler==k)return e;i.push({handler:k,on:true});return e};e.off=function(c,k){var i=f[c];if(i)for(var a=0;a<i.length;a++){var g=i[a];if(g.handler==k){g.on=false;i.splice(a,1);break}}return e};return function(c){var k=
f[c.type];if(k){k=k.slice();for(var i=0;i<k.length;i++){var a=k[i];a.on&&a.handler.call(e,c)}}}};t.queue=function(){function e(){if(!(i>=a||!k.length)){i++;k.pop()()}}function f(g){for(var n=0;n<k.length;n++)if(k[n]==g){k.splice(n,1);return true}return false}function c(g,n,l){function s(){m=new XMLHttpRequest;l&&m.overrideMimeType(l);m.open("GET",g,true);m.onreadystatechange=function(){if(m.readyState==4){i--;m.status<300&&n(m);e()}};m.send(null)}var m;k.push(s);e();return{abort:function(j){if(f(s))return true;
if(j&&m){m.abort();return true}return false}}}var k=[],i=0,a=6;return{text:function(g,n,l){return c(g,function(s){s.responseText&&n(s.responseText)},l)},xml:function(g,n){return c(g,function(l){l.responseXML&&n(l.responseXML)},"application/xml")},json:function(g,n){return c(g,function(l){l.responseText&&n(JSON.parse(l.responseText))},"application/json")},image:function(g,n,l){function s(){m=document.createElement("img");m.onerror=function(){i--;e()};m.onload=function(){i--;l(m);e()};m.src=n;g.setAttributeNS(t.ns.xlink,
"href",n)}var m;k.push(s);e();return{abort:function(j){if(f(s))return true;if(j&&m){m.src="about:";return true}return false}}}}}();t.map=function(){function e(){if(u)if(s<u[0])s=u[0];else if(s>u[1])s=u[1];m=s-(s=Math.round(s));j=Math.pow(2,m)}function f(){if(o){var d=45/Math.pow(2,s+m-3),r=Math.max(Math.abs(C*g.x+A*g.y),Math.abs(B*g.x+E*g.y)),q=V(K-r*d/n.y);r=V(h+r*d/n.y);l.lat=Math.max(q,Math.min(r,l.lat));q=Math.max(Math.abs(C*g.y+A*g.x),Math.abs(B*g.y+E*g.x));l.lon=Math.max(o[0].lon-q*d/n.x,Math.min(o[1].lon+
q*d/n.x,l.lon))}}var c={},k,i,a=Y,g=Y,n={x:256,y:256},l={lat:37.76487,lon:-122.41948},s=12,m=0,j=1,u=[1,18],y=0,A=1,C=0,E=1,B=0,K=-180,h=180,o=[{lat:V(K),lon:-Infinity},{lat:V(h),lon:Infinity}];c.locationCoordinate=function(d){d=t.map.locationCoordinate(d);var r=Math.pow(2,s);d.column*=r;d.row*=r;d.zoom+=s;return d};c.coordinateLocation=t.map.coordinateLocation;c.coordinatePoint=function(d,r){var q=Math.pow(2,s-r.zoom),z=Math.pow(2,s-d.zoom),v=(r.column*q-d.column*z)*n.x*j;q=(r.row*q-d.row*z)*n.y*
j;return{x:g.x+A*v-C*q,y:g.y+C*v+A*q}};c.pointCoordinate=function(d,r){var q=Math.pow(2,s-d.zoom),z=(r.x-g.x)/j,v=(r.y-g.y)/j;return{column:d.column*q+(E*z-B*v)/n.x,row:d.row*q+(B*z+E*v)/n.y,zoom:s}};c.locationPoint=function(d){var r=Math.pow(2,s+m-3)/45,q=(d.lon-l.lon)*r*n.x;d=(W(l.lat)-W(d.lat))*r*n.y;return{x:g.x+A*q-C*d,y:g.y+C*q+A*d}};c.pointLocation=function(d){var r=45/Math.pow(2,s+m-3),q=(d.x-g.x)*r;d=(d.y-g.y)*r;return{lon:l.lon+(E*q-B*d)/n.x,lat:V(W(l.lat)-(B*q+E*d)/n.y)}};var w=t.svg("rect");
...
return c};c.tileSize=function(d){if(!arguments.length)return n;n=d;c.dispatch({type:"move"});return c};c.center=function(d){if(!arguments.length)return l;l=d;f();c.dispatch({type:"move"});return c};c.panBy=function(d){var r=45/Math.pow(2,s+m-3),q=d.x*r;d=d.y*r;return c.center({lon:l.lon+(B*d-E*q)/n.x,lat:V(W(l.lat)+(B*q+E*d)/n.y)})};c.centerRange=function(d){if(!arguments.length)return o;if(o=d){K=o[0].lat>-90?W(o[0].lat):-Infinity;h=o[0].lat<90?W(o[1].lat):Infinity}else{K=-Infinity;h=Infinity}f();
c.dispatch({type:"move"});return c};c.zoom=function(d){if(!arguments.length)return s+m;s=d;e();return c.center(l)};c.zoomBy=function(d,r,q){if(arguments.length<2)return c.zoom(s+m+d);if(arguments.length<3)q=c.pointLocation(r);s=s+m+d;e();var z=c.locationPoint(q);return c.panBy({x:r.x-z.x,y:r.y-z.y})};c.zoomRange=function(d){if(!arguments.length)return u;u=d;return c.zoom(s+m)};c.extent=function(d){if(!arguments.length)return[c.pointLocation({x:0,y:a.y}),c.pointLocation({x:a.x,y:0})];var r=c.locationPoint(d[0]),
q=c.locationPoint(d[1]),z=Math.max((q.x-r.x)/a.x,(r.y-q.y)/a.y);r=c.pointLocation({x:(r.x+q.x)/2,y:(r.y+q.y)/2});s=s+m-Math.log(z)/Math.log(2);e();return c.center(r)};c.angle=function(d){if(!arguments.length)return y;y=d;A=Math.cos(y);C=Math.sin(y);E=Math.cos(-y);B=Math.sin(-y);f();c.dispatch({type:"move"});return c};c.add=function(d){d.map(c);return c};c.remove=function(d){d.map(null);return c};c.dispatch=t.dispatch(c);return c};S.maps=[];S.add=function(e){for(var f=0;f<S.maps.length;f++)if(S.maps[f]==
...
0;if(l&&R>-5&&R<3){r=I.zoom<0?1:1<<I.zoom;if(n){da(I,v,o,0,r,K);da(o,h,I,0,r,K)}else{d=Math.floor((I.column+o.column)/2);r=Math.max(0,Math.min(r-1,Math.floor((v.row+h.row)/2)));v=Math.min(4,I.zoom);d=d>>v<<v;r=r>>v<<v;K(d,d+1,r)}}for(F in M){v=M[F];d=Math.pow(2,v.level=v.zoom-z.zoom);v.element.setAttribute("transform","translate("+(v.x=q.x*(v.column-z.column*d))+","+(v.y=q.y*(v.row-z.row*d))+")")}for(F in w)if(!(F in M)){v=g.unload(F);v.element.parentNode.removeChild(v.element);delete v.proxyRefs}for(F in M){v=
M[F];if(v.element.parentNode!=C[v.level]){C[v.level].appendChild(v.element);a.show&&a.show(v)}}g.flush();a.dispatch({type:"move"})}var a={},g=a.cache=t.cache(e,f).size(512),n=true,l=true,s,m,j,u=t.svg("g"),y,A,C={};u.setAttribute("class","layer");for(var E=-4;E<=-1;E++)C[E]=u.appendChild(t.svg("g"));for(E=2;E>=1;E--)C[E]=u.appendChild(t.svg("g"));C[0]=u.appendChild(t.svg("g"));a.map=function(B){if(!arguments.length)return j;if(j){if(j==B){u.parentNode.appendChild(u);return a}j.off("move",i).off("resize",
i);u.parentNode.removeChild(u)}if(j=B){j.container().appendChild(u);a.init&&a.init(u);j.on("move",i).on("resize",i);i()}return a};a.container=function(){return u};a.levels=function(){return C};a.id=function(B){if(!arguments.length)return m;m=B;u.setAttribute("id",B);return a};a.visible=function(B){if(!arguments.length)return l;(l=B)?u.removeAttribute("visibility"):u.setAttribute("visibility","hidden");j&&i();return a};a.transform=function(B){if(!arguments.length)return y;y=B;j&&i();return a};a.zoom=
...
break;default:(L=c(r,o))&&q.push({element:d.appendChild(L),data:{type:"Feature",geometry:r}});break}h.ready=true;q.push.apply(h.features,q);a.dispatch({type:"load",tile:h,features:q})}var d=h.element=t.svg("g");h.features=[];o=f(o(h).locationPoint);if(n!=null)h.request=e(typeof n=="function"?n(h):n,w);else w({type:"FeatureCollection",features:A||[]})},function(h){h.request&&h.request.abort(true)}),g=a.container(),n,l=true,s="org.polymaps."+t.id(),m="url(#"+s+")",j=g.insertBefore(t.svg("clipPath"),
...
t.url(h):h;if(n!=null)A=null;typeof n=="string"&&a.tile(false);return a.reload()};a.features=function(h){if(!arguments.length)return A;if(A=h){n=null;a.tile(false)}return a.reload()};a.clip=function(h){if(!arguments.length)return l;l&&g.removeChild(j);if(l=h)g.insertBefore(j,g.firstChild);var o=a.cache.locks();for(var w in o)l?o[w].element.setAttribute("clip-path",m):o[w].element.removeAttribute("clip-path");return a};var B=a.tile;a.tile=function(h){arguments.length&&!h&&a.clip(h);return B.apply(a,
arguments)};var K=a.map;a.map=function(h){if(h&&u){var o=h.tileSize();u.setAttribute("width",o.x);u.setAttribute("height",o.y)}return K.apply(a,arguments)};a.scale=function(h){if(!arguments.length)return y;(y=h)?a.on("move",i):a.off("move",i);a.map()&&i();return a};a.show=function(h){l?h.element.setAttribute("clip-path",m):h.element.removeAttribute("clip-path");a.dispatch({type:"show",tile:h,features:h.features});return a};a.reshow=function(){var h=a.cache.locks();for(var o in h)a.show(h[o]);return a};
...
false);return c};t.wheel=function(){function e(){n=null}function f(m){var j=(m.wheelDelta/120||-m.detail)*0.1,u;if(ba<0){u=Date.now();var y=u-i;if(y>9&&Math.abs(m.wheelDelta)/y>=50)ba=1;i=u}if(ba==1)j*=0.03;if(!a&&j){u=Date.now();if(u-k>200){j=j>0?+1:-1;k=u}else j=0}if(j)switch(g){case "mouse":u=l.mouse(m);n||(n=l.pointLocation(u));l.off("move",e).zoomBy(j,u,n).on("move",e);break;case "location":l.zoomBy(j,l.locationPoint(n),n);break;default:l.zoomBy(j);break}m.preventDefault();return false}var c=
{},k=0,i=0,a=true,g="mouse",n,l,s;c.smooth=function(m){if(!arguments.length)return a;a=m;return c};c.zoom=function(m,j){if(!arguments.length)return g;g=m;n=j;if(l)g=="mouse"?l.on("move",e):l.off("move",e);return c};c.map=function(m){if(!arguments.length)return l;if(l){s.removeEventListener("mousemove",e,false);s.removeEventListener("mousewheel",f,false);s.removeEventListener("DOMMouseScroll",f,false);s=null;l.off("move",e)}if(l=m){g=="mouse"&&l.on("move",e);s=l.container();s.addEventListener("mousemove",
...
g+l)){var y=(a.left-a.right)*m,A=(a.up-a.down)*m;if(y||A)j.panBy({x:y,y:A})}}var i={},a={left:0,right:0,up:0,down:0},g=0,n,l=250,s=50,m=16,j,u;i.map=function(y){if(!arguments.length)return j;if(j){u.removeEventListener("keypress",c,false);u.removeEventListener("keydown",e,false);u.removeEventListener("keyup",f,false);u=null}if(j=y){u=j.focusableParent();u.addEventListener("keypress",c,false);u.addEventListener("keydown",e,false);u.addEventListener("keyup",f,false)}return i};i.speed=function(y){if(!arguments.length)return m;
m=y;return i};return i};t.hash=function(){function e(){var l=n(a);if(k!==l)location.replace(k=l)}function f(){if(location.hash!==k){var l=g((k=location.hash).substring(1));if(!l||l.length==undefined||l.length<3||l.some(isNaN))e();else{var s=a.size();a.zoomBy(l[0]-a.zoom(),{x:s.x/2,y:s.y/2},{lat:Math.min(i,Math.max(-i,l[1])),lon:l[2]})}}}var c={},k,i=89.99999999,a,g=function(l){return l.split("/").map(Number)},n=function(l){var s=l.center();l=l.zoom();var m=Math.max(0,Math.ceil(Math.log(l)/Math.LN2));
return"#"+l.toFixed(2)+"/"+s.lat.toFixed(m)+"/"+s.lon.toFixed(m)};c.map=function(l){if(!arguments.length)return a;if(a){a.off("move",e);window.removeEventListener("hashchange",f,false)}if(a=l){a.on("move",e);window.addEventListener("hashchange",f,false);location.hash?f():e()}return c};c.parse=function(l){if(!arguments.length)return g;g=l;return c};c.format=function(l){if(!arguments.length)return n;n=l;return c};return c};t.interact=function(){var e={},f=t.drag(),c=t.wheel(),k=t.dblclick(),i=t.arrow();
e.map=function(a){f.map(a);c.map(a);k.map(a);i.map(a);return e};return e};t.compass=function(){function e(x){B.setAttribute("class","compass active");I||(I=setInterval(f,r));R&&H.panBy(R);w=Date.now();return m(x)}function f(){R&&Date.now()>w+d&&H.panBy(R)}function c(x){if(x.shiftKey){F={x0:H.mouse(x)};H.focusableParent().focus();return m(x)}}function k(x){if(F){F.x1=H.mouse(x);O.setAttribute("x",Math.min(F.x0.x,F.x1.x));O.setAttribute("y",Math.min(F.x0.y,F.x1.y));O.setAttribute("width",Math.abs(F.x0.x-
F.x1.x));O.setAttribute("height",Math.abs(F.x0.y-F.x1.y));O.removeAttribute("display")}}function i(){B.setAttribute("class","compass");if(F){if(F.x1){H.extent([H.pointLocation({x:Math.min(F.x0.x,F.x1.x),y:Math.max(F.x0.y,F.x1.y)}),H.pointLocation({x:Math.max(F.x0.x,F.x1.x),y:Math.min(F.x0.y,F.x1.y)})]);O.setAttribute("display","none")}F=null}if(I){clearInterval(I);I=0}}function a(x){return function(){x?this.setAttribute("class","active"):this.removeAttribute("class");R=x}}function g(x){return function(D){B.setAttribute("class",
"compass active");var G=H.zoom();H.zoom(x<0?Math.ceil(G)-1:Math.floor(G)+1);return m(D)}}function n(x){return function(D){H.zoom(x);return m(D)}}function l(){this.setAttribute("class","active")}function s(){this.removeAttribute("class")}function m(x){x.stopPropagation();x.preventDefault();return false}function j(x){var D=Math.SQRT1_2*h,G=h*0.7,J=h*0.2,N=t.svg("g"),P=N.appendChild(t.svg("path")),Q=N.appendChild(t.svg("path"));P.setAttribute("class","direction");P.setAttribute("pointer-events","all");
P.setAttribute("d","M0,0L"+D+","+D+"A"+h+","+h+" 0 0,1 "+-D+","+D+"Z");Q.setAttribute("class","chevron");Q.setAttribute("d","M"+J+","+(G-J)+"L0,"+G+" "+-J+","+(G-J));Q.setAttribute("pointer-events","none");N.addEventListener("mousedown",e,false);N.addEventListener("mouseover",a(x),false);N.addEventListener("mouseout",a(null),false);N.addEventListener("dblclick",m,false);return N}function u(x){var D=h*0.4,G=D/2,J=t.svg("g"),N=J.appendChild(t.svg("path")),P=J.appendChild(t.svg("path")),Q=J.appendChild(t.svg("path")),
X=J.appendChild(t.svg("path"));N.setAttribute("class","back");N.setAttribute("d","M"+-D+",0V"+-D+"A"+D+","+D+" 0 1,1 "+D+","+-D+"V0Z");P.setAttribute("class","direction");P.setAttribute("d",N.getAttribute("d"));Q.setAttribute("class","chevron");Q.setAttribute("d","M"+-G+","+-D+"H"+G+(x>0?"M0,"+(-D-G)+"V"+-G:""));X.setAttribute("class","fore");X.setAttribute("fill","none");X.setAttribute("d",N.getAttribute("d"));J.addEventListener("mousedown",g(x),false);J.addEventListener("mouseover",l,false);J.addEventListener("mouseout",
s,false);J.addEventListener("dblclick",m,false);return J}function y(x){var D=h*0.2,G=h*0.4,J=t.svg("g"),N=J.appendChild(t.svg("rect")),P=J.appendChild(t.svg("path"));N.setAttribute("pointer-events","all");N.setAttribute("fill","none");N.setAttribute("x",-G);N.setAttribute("y",-0.75*G);N.setAttribute("width",2*G);N.setAttribute("height",1.5*G);P.setAttribute("class","chevron");P.setAttribute("d","M"+-D+",0H"+D);J.addEventListener("mousedown",n(x),false);J.addEventListener("dblclick",m,false);return J}
...
i,false);H.on("move",A).on("resize",A);C()}return E};return E};t.grid=function(){function e(){var i=k.firstChild,a=c.size(),g=c.pointLocation(Y);c.pointLocation(a);var n=Math.pow(2,4-Math.round(c.zoom()));g.lat=Math.floor(g.lat/n)*n;g.lon=Math.ceil(g.lon/n)*n;for(var l;(l=c.locationPoint(g).x)<=a.x;g.lon+=n){i||(i=k.appendChild(t.svg("line")));i.setAttribute("x1",l);i.setAttribute("x2",l);i.setAttribute("y1",0);i.setAttribute("y2",a.y);i=i.nextSibling}for(;(l=c.locationPoint(g).y)<=a.y;g.lat-=n){i||
(i=k.appendChild(t.svg("line")));i.setAttribute("y1",l);i.setAttribute("y2",l);i.setAttribute("x1",0);i.setAttribute("x2",a.x);i=i.nextSibling}for(;i;){a=i.nextSibling;k.removeChild(i);i=a}}var f={},c,k=t.svg("g");k.setAttribute("class","grid");f.map=function(i){if(!arguments.length)return c;if(c){k.parentNode.removeChild(k);c.off("move",e).off("resize",e)}if(c=i){c.on("move",e).on("resize",e);c.container().appendChild(k);c.dispatch({type:"move"})}return f};return f};t.stylist=function(){function e(i){var a=
i.features.length,g=f.length,n=c.length,l,s,m,j,u,y;for(u=0;u<a;++u)if(s=(l=i.features[u]).element){l=l.data;for(y=0;y<g;++y){j=(m=f[y]).value;if(typeof j==="function")j=j.call(null,l);j==null?m.name.local?s.removeAttributeNS(m.name.space,m.name.local):s.removeAttribute(m.name):m.name.local?s.setAttributeNS(m.name.space,m.name.local,j):s.setAttribute(m.name,j)}for(y=0;y<n;++y){j=(m=c[y]).value;if(typeof j==="function")j=j.call(null,l);j==null?s.style.removeProperty(m.name):s.style.setProperty(m.name,
j,m.priority)}if(j=k){if(typeof j==="function")j=j.call(null,l);for(;s.lastChild;)s.removeChild(s.lastChild);j!=null&&s.appendChild(t.svg("title")).appendChild(document.createTextNode(j))}}}var f=[],c=[],k;e.attr=function(i,a){f.push({name:fa(i),value:a});return e};e.style=function(i,a,g){c.push({name:i,value:a,priority:arguments.length<3?null:g});return e};e.title=function(i){k=i;return e};return e}})(org.polymaps);
