...
D);s.insertBefore(w[2],D);s.insertBefore(w[1],D);for(var x=w[-4],m=-4;m<2;)w[m]=w[++m];w[m]=x}}function j(B){for(var D=w[0].nextSibling;K>B;K--){s.insertBefore(w[-1],D);s.insertBefore(w[2],w[-4]);for(var x=w[2],m=2;m>-4;)w[m]=w[--m];w[m]=x}}function h(){function B(E){var T=E.zoom,U=T<0?1:1<<T,t=E.column%U,C=E.row;if(t<0)t+=U;return{locationPoint:function(L){L=r.map.locationCoordinate(L);var G=Math.pow(2,T-L.zoom);return{x:v.x*(G*L.column-t),y:v.y*(G*L.row-C)}}}}function D(E,T,U){var t=H.zoom,C=2-
...
var x=c.map(),m=x.zoom(),g=m-(m=Math.round(m)),p=x.size(),n=x.angle(),v=x.tileSize(),A=x.locationCoordinate(x.center());if(K!=m){if(K<m)a(m);else if(K>m)j(m);else K=m;for(var z=-4;z<=2;z++){var I=w[z];I.setAttribute("class","zoom"+(z<0?"":"+")+z+" zoom"+(m+z));I.setAttribute("transform","scale("+Math.pow(2,-z)+")")}}s.setAttribute("transform","translate("+p.x/2+","+p.y/2+")"+(n?"rotate("+n/Math.PI*180+")":"")+(g?"scale("+Math.pow(2,g)+")":"")+(u?u.zoomFraction(g):""));var H=x.pointCoordinate(A,X);
...
J[P];if(z.element.parentNode!=w[z.level]){w[z.level].appendChild(z.element);c.show&&c.show(z)}}e.flush();m+=g;if(y!=m){y=m;c.dispatch({type:"zoom"})}}var c={},e=c.cache=r.cache(d,f).size(512),o=true,k=true,q,l,i,s=r.svg("g"),u,y,K,w={};s.setAttribute("class","layer");for(var F=-4;F<=-1;F++)w[F]=s.appendChild(r.svg("g"));for(F=2;F>=1;F--)w[F]=s.appendChild(r.svg("g"));w[0]=s.appendChild(r.svg("g"));c.map=function(B){if(!arguments.length)return i;if(i){if(i==B){s.parentNode.appendChild(s);return c}i.off("move",
h).off("resize",h);s.parentNode.removeChild(s)}if(i=B){i.container().appendChild(s);c.init&&c.init(s);i.on("move",h).on("resize",h);h()}return c};c.container=function(){return s};c.id=function(B){if(!arguments.length)return l;l=B;s.setAttribute("id",B);return c};c.visible=function(B){if(!arguments.length)return k;(k=B)?s.removeAttribute("visibility"):s.setAttribute("visibility","hidden");i&&h();return c};c.transform=function(B){if(!arguments.length)return u;u=B;i&&h();return c};c.zoom=function(B){if(!arguments.length)return q;
...
h=d.map().tileSize();j.setAttribute("preserveAspectRatio","none");j.setAttribute("width",h.x);j.setAttribute("height",h.y);if(typeof f=="function"){j.setAttribute("opacity",0);a.request=r.queue.image(j,f(a),function(c){delete a.request;a.ready=true;a.img=c;j.removeAttribute("opacity");d.dispatch({type:"load",tile:a})})}else{a.ready=true;f&&j.setAttributeNS(r.ns.xlink,"href",f);d.dispatch({type:"load",tile:a})}},function(a){a.request&&a.request.abort(true)}),f;d.url=function(a){if(!arguments.length)return f;
f=typeof a=="string"&&/{.}/.test(a)?r.url(a):a;return d.reload()};return d};r.geoJson=function(d){function f(m){var g={lat:0,lon:0};return function(p){g.lat=p[1];g.lon=p[0];var n=m(g);p.x=n.x;p.y=n.y;return n}}function a(m,g){return m&&m.type in B&&B[m.type](m,g)}function j(m,g){var p=g(m),n=r.svg("circle");n.setAttribute("r",4.5);n.setAttribute("transform","translate("+p.x+","+p.y+")");return n}function h(m,g,p,n){n.push("M");for(var v=0;v<m.length-g;v++){var A=p(m[v]);n.push(A.x,",",A.y,"L")}n.pop()}
function c(m,g,p,n){for(var v=0;v<m.length;v++)h(m[v],g,p,n);g&&n.push("Z")}function e(m,g,p,n){for(var v=[],A=0;A<g.length;A++)m(g[A],p,n,v);if(v.length){m=r.svg("path");m.setAttribute("d",v.join(""));return m}}function o(){if(w=="fixed"){var m=k.cache.locks(),g=k.map().zoom(),p=["translate(",null,",",null,null],n,v,A,z,I,H,N;for(A in m){p[4]=")scale("+Math.pow(2,(n=m[A]).zoom-g)+")";z=n.features;for(n=0;n<z.length;n++){H=(I=z[n]).element;I=(v=I.data.geometry).coordinates;switch(v.type){case "Point":p[1]=
I.x;p[3]=I.y;H.setAttribute("transform",p.join(""));break;case "MultiPoint":v=0;for(H=H.firstChild;v<I.length;v++,H=H.nextSibling){p[1]=(N=I[v]).x;p[3]=N.y;H.setAttribute("transform",p.join(""))}break}}}}}var k=r.layer(function(m,g){function p(v){var A=[];if(v.next)m.request=d(v.next.href,p);switch(v.type){case "FeatureCollection":for(var z=0;z<v.features.length;z++){var I=v.features[z],H=a(I.geometry,g);H&&A.push({element:n.appendChild(H),data:I})}break;case "Feature":(H=a(v.geometry,g))&&A.push({element:n.appendChild(H),
data:v});break;default:(H=a(v,g))&&A.push({element:n.appendChild(H),data:{type:"Feature",geometry:v}});break}m.ready=true;A.push.apply(m.features,A);k.dispatch({type:"load",tile:m,features:A})}var n=m.element=r.svg("g");m.features=[];g=f(g(m).locationPoint);if(l!=null)m.request=d(typeof l=="function"?l(m):l,p);else p({type:"FeatureCollection",features:F||[]})},function(m){m.request&&m.request.abort(true)}),q=k.container(),l,i=true,s="org.polymaps."+r.id(),u="url(#"+s+")",y=q.insertBefore(r.svg("clipPath"),
q.firstChild),K=y.appendChild(r.svg("rect")),w="auto",F;q.setAttribute("fill-rule","evenodd");y.setAttribute("id",s);if(!arguments.length)d=r.queue.json;var B={Point:function(m,g){return j(m.coordinates,g)},MultiPoint:function(m,g){for(var p=r.svg("g"),n=0;n<m.coordinates.length;n++)p.appendChild(j(m.coordinates[n],g));return p},LineString:function(m,g){return e(h,[m.coordinates],0,g)},MultiLineString:function(m,g){return e(h,m.coordinates,0,g)},Polygon:function(m,g){return e(c,[m.coordinates],1,
g)},MultiPolygon:function(m,g){return e(c,m.coordinates||m.coords,1,g)},GeometryCollection:function(m,g){for(var p=r.svg("g"),n=0;n<m.geometries.length;n++){var v=a(m.geometries[n],g);v&&p.appendChild(v)}return p}};k.url=function(m){if(!arguments.length)return l;l=typeof m=="string"&&/{.}/.test(m)?r.url(m):m;if(l!=null)F=null;typeof l=="string"&&k.tile(false);return k.reload()};k.features=function(m){if(!arguments.length)return F;if(F=m){l=null;k.tile(false)}return k.reload()};k.clip=function(m){if(!arguments.length)return i;
i&&q.removeChild(y);if(i=m)q.insertBefore(y,q.firstChild);var g=k.cache.locks();for(var p in g)i?g[p].element.setAttribute("clip-path",u):g[p].element.removeAttribute("clip-path");return k};var D=k.tile;k.tile=function(m){arguments.length&&!m&&k.clip(m);return D.apply(k,arguments)};var x=k.map;k.map=function(m){if(m&&K){var g=m.tileSize();K.setAttribute("width",g.x);K.setAttribute("height",g.y)}return x.apply(k,arguments)};k.scale=function(m){if(!arguments.length)return w;(w=m)?k.on("zoom",o):k.off("zoom",
o);return k};k.show=function(m){i?m.element.setAttribute("clip-path",u):m.element.removeAttribute("clip-path");k.dispatch({type:"show",tile:m,features:m.features});return k};k.reshow=function(){var m=k.cache.locks();for(var g in m)k.show(m[g]);return k};return k};r.dblclick=function(){function d(c){var e=j.zoom();e=c.shiftKey?Math.ceil(e)-e-1:1-e+Math.floor(e);a==="mouse"?j.zoomBy(e,j.mouse(c)):j.zoomBy(e)}var f={},a="mouse",j,h;f.zoom=function(c){if(!arguments.length)return a;a=c;return f};f.map=
...
u.altKey||u.metaKey)){var y=Date.now(),K=0,w=0;switch(u.keyCode){case 37:if(!c.left){e=y;c.left=1;c.right||(K=l)}break;case 39:if(!c.right){e=y;c.right=1;c.left||(K=-l)}break;case 38:if(!c.up){e=y;c.up=1;c.down||(w=l)}break;case 40:if(!c.down){e=y;c.down=1;c.up||(w=-l)}break;default:return}if(K||w)i.panBy({x:K,y:w});if(!o&&c.left|c.right|c.up|c.down)o=setInterval(j,q);u.preventDefault()}}function f(u){e=Date.now();switch(u.keyCode){case 37:c.left=0;break;case 39:c.right=0;break;case 38:c.up=0;break;
case 40:c.down=0;break;default:return}if(o&&!(c.left|c.right|c.up|c.down))o=clearInterval(o);u.preventDefault()}function a(u){switch(u.charCode){case 45:case 95:i.zoom(Math.ceil(i.zoom())-1);break;case 43:case 61:i.zoom(Math.floor(i.zoom())+1);break;default:return}u.preventDefault()}function j(){if(i)if(!(Date.now()<e+k)){var u=(c.left-c.right)*l,y=(c.up-c.down)*l;if(u||y)i.panBy({x:u,y:y})}}var h={},c={left:0,right:0,up:0,down:0},e=0,o,k=250,q=50,l=16,i,s;h.map=function(u){if(!arguments.length)return i;
if(i){s.removeEventListener("keypress",a,false);s.removeEventListener("keydown",d,false);s.removeEventListener("keyup",f,false);s=null}if(i=u){s=i.focusableParent();s.addEventListener("keypress",a,false);s.addEventListener("keydown",d,false);s.addEventListener("keyup",f,false)}return h};h.speed=function(u){if(!arguments.length)return l;l=u;return h};return h};r.hash=function(){function d(){var e=c.center(),o=c.zoom(),k=Math.max(0,Math.ceil(Math.log(o)/Math.LN2));e="#"+o.toFixed(2)+"/"+e.lat.toFixed(k)+
...
location.hash?f():d()}return a};return a};r.interact=function(){var d={},f=r.drag(),a=r.wheel(),j=r.dblclick(),h=r.arrow();d.map=function(c){f.map(c);a.map(c);j.map(c);h.map(c);return d};return d};r.compass=function(){function d(t){F.setAttribute("class","compass active");I||(I=setInterval(f,p));H&&E.panBy(H);m=Date.now();return l(t)}function f(){H&&Date.now()>m+g&&E.panBy(H)}function a(t){if(t.shiftKey){J={x0:E.mouse(t)};E.focusableParent().focus();return l(t)}}function j(t){if(J){J.x1=E.mouse(t);
...
A.appendChild(s(-1)).setAttribute("transform","scale(-1)")}else A=null;y()}var w={},F=r.svg("g"),B={},D=30,x=16,m=0,g=250,p=50,n="top-left",v="small",A,z="small",I,H,N,J,P=r.svg("rect"),E,T,U;F.setAttribute("class","compass");P.setAttribute("class","back fore");P.setAttribute("pointer-events","none");P.setAttribute("display","none");w.radius=function(t){if(!arguments.length)return D;D=t;E&&K();return w};w.speed=function(t){if(!arguments.length)return D;x=t;return w};w.position=function(t){if(!arguments.length)return n;
...
return f};r.stylist=function(){function d(h){var c=h.features.length,e=f.length,o=a.length,k,q,l,i,s,u;for(s=0;s<c;++s)if(q=(k=h.features[s]).element){k=k.data;for(u=0;u<e;++u){i=(l=f[u]).value;if(typeof i==="function")i=i.call(null,k);i==null?l.name.local?q.removeAttributeNS(l.name.space,l.name.local):q.removeAttribute(l.name):l.name.local?q.setAttributeNS(l.name.space,l.name.local,i):q.setAttribute(l.name,i)}for(u=0;u<o;++u){i=(l=a[u]).value;if(typeof i==="function")i=i.call(null,k);i==null?q.style.removeProperty(l.name):
q.style.setProperty(l.name,i,l.priority)}if(i=j){if(typeof i==="function")i=i.call(null,k);for(;q.lastChild;)q.removeChild(q.lastChild);i!=null&&q.appendChild(r.svg("title")).appendChild(document.createTextNode(i))}}}var f=[],a=[],j;d.attr=function(h,c){f.push({name:fa(h),value:c});return d};d.style=function(h,c,e){a.push({name:h,value:c,priority:arguments.length<3?null:e});return d};d.title=function(h){j=h;return d};return d}})(org.polymaps);
