...
a.dx/a.dy,m=g.dx>0,q=a.dx<0;for(e=e;e<d;e++){var u=f*Math.max(0,Math.min(g.dy,e+m-g.y0))+g.x0;c(Math.floor(i*Math.max(0,Math.min(a.dy,e+q-a.y0))+a.x0),Math.ceil(u),e)}}function $(g,a,e,d,c,f){var i=U(g,a);a=U(a,e);g=U(e,g);if(i.dy>a.dy){e=i;i=a;a=e}if(i.dy>g.dy){e=i;i=g;g=e}if(a.dy>g.dy){e=a;a=g;g=e}i.dy&&Z(g,i,d,c,f);a.dy&&Z(g,a,d,c,f)}r.version="2.0.2+3";var R={x:0,y:0};r.id=function(){var g=0;return function(){return++g}}();r.svg=function(g){return document.createElementNS(r.ns.svg,g)};r.ns={svg:"http://www.w3.org/2000/svg",
xlink:"http://www.w3.org/1999/xlink"};r.transform=function(g,a,e,d,c,f){var i={},m,q,u;if(!arguments.length){g=1;a=c=e=0;d=1;f=0}i.zoomFraction=function(n){if(!arguments.length)return q;q=n;m=Math.floor(q+Math.log(Math.sqrt(g*g+a*a+e*e+d*d))/Math.log(2));u=Math.pow(2,-m);return i};i.apply=function(n){var l=Math.pow(2,-n.zoom),h=Math.pow(2,n.zoom-m);return{column:(g*n.column*l+e*n.row*l+c)*h,row:(a*n.column*l+d*n.row*l+f)*h,zoom:n.zoom-m}};i.unapply=function(n){var l=Math.pow(2,-n.zoom),h=Math.pow(2,
n.zoom+m);return{column:(n.column*l*d-n.row*l*e-c*d+f*e)/(g*d-a*e)*h,row:(n.column*l*a-n.row*l*g-c*a+f*g)/(e*a-d*g)*h,zoom:n.zoom+m}};i.toString=function(){return"matrix("+[g*u,a*u,e*u,d*u].join(" ")+" 0 0)"};return i.zoomFraction(0)};r.cache=function(g,a){function e(l){n--;a&&a(l);delete i[l.key];if(l.next)l.next.prev=l.prev;else if(q=l.prev)q.next=null;if(l.prev)l.prev.next=l.next;else if(m=l.next)m.prev=null}function d(){for(var l=q;n>u;l=l.prev){if(!l)break;l.lock||e(l)}}var c={},f={},i={},m=
null,q=null,u=64,n=0;c.peek=function(l){return i[[l.zoom,l.column,l.row].join("/")]};c.load=function(l,h){var o=[l.zoom,l.column,l.row].join("/"),s=i[o];if(s){if(s.prev){if(s.prev.next=s.next)s.next.prev=s.prev;else q=s.prev;s.prev=null;s.next=m;m=m.prev=s}s.lock=1;return f[o]=s}s={key:o,column:l.column,row:l.row,zoom:l.zoom,next:m,prev:null,lock:1};g.call(null,s,h);f[o]=i[o]=s;if(m)m.prev=s;else q=s;m=s;n++;d();return s};c.unload=function(l){if(!(l in f))return false;var h=f[l];h.lock=0;delete f[l];
h.request&&h.request.abort(false)?e(h):d();return h};c.locks=function(){return f};c.size=function(l){if(!arguments.length)return u;u=l;d();return c};return c};r.url=function(g){function a(d){var c=1<<d.zoom,f=d.column%c;if(f<0)f+=c;return g.replace(/{(.)}/g,function(i,m){switch(m){case "S":return e[(d.zoom+d.row+f)%e.length];case "Z":return d.zoom;case "X":return f;case "Y":return d.row;case "B":var q=r.map.coordinateLocation({row:d.row,column:f,zoom:d.zoom}),u=r.map.coordinateLocation({row:d.row+
1,column:f+1,zoom:d.zoom}),n=Math.ceil(Math.log(d.zoom)/Math.LN2);return u.lat.toFixed(n)+","+q.lon.toFixed(n)+","+q.lat.toFixed(n)+","+u.lon.toFixed(n)}return m})}var e=[];a.template=function(d){if(!arguments.length)return g;g=d;return a};a.hosts=function(d){if(!arguments.length)return e;e=d;return a};return a};r.dispatch=function(g){var a={};g.on=function(e,d){for(var c=a[e]||(a[e]=[]),f=0;f<c.length;f++)if(c[f].handler==d)return g;c.push({handler:d,on:true});return g};g.off=function(e,d){var c=
a[e];if(c)for(var f=0;f<c.length;f++){var i=c[f];if(i.handler==d){i.on=false;c.splice(f,1);break}}return g};return function(e){var d=a[e.type];if(d){d=d.slice();for(var c=0;c<d.length;c++){var f=d[c];f.on&&f.handler.call(g,e)}}}};r.queue=function(){function g(){if(!(c>=f||!d.length)){c++;d.pop()()}}function a(i){for(var m=0;m<d.length;m++)if(d[m]==i){d.splice(m,1);return true}return false}function e(i,m,q){function u(){n=new XMLHttpRequest;q&&n.overrideMimeType(q);n.open("GET",i,true);n.onreadystatechange=
function(){if(n.readyState==4){c--;n.status<300&&m(n);g()}};n.send(null)}var n;d.push(u);g();return{abort:function(l){if(a(u))return true;if(l&&n){n.abort();return true}return false}}}var d=[],c=0,f=6;return{text:function(i,m,q){return e(i,function(u){u.responseText&&m(u.responseText)},q)},xml:function(i,m){return e(i,function(q){q.responseXML&&m(q.responseXML)},"application/xml")},json:function(i,m){return e(i,function(q){q.responseText&&m(JSON.parse(q.responseText))},"application/json")},image:function(i,
m,q){function u(){n=document.createElement("img");n.onerror=function(){c--;g()};n.onload=function(){c--;q(n);g()};n.src=m;i.setAttributeNS(r.ns.xlink,"href",m)}var n;d.push(u);g();return{abort:function(l){if(a(u))return true;if(l&&n){n.src="about:";return true}return false}}}}}();r.map=function(){function g(){if(G){var j=45/Math.pow(2,q+u-3),t=Math.max(Math.abs(s*f.x+o*f.y),Math.abs(x*f.x+k*f.y)),w=P(B-t*j/i.y);t=P(H+t*j/i.y);m.lat=Math.max(w,Math.min(t,m.lat));w=Math.max(Math.abs(s*f.y+o*f.x),Math.abs(x*
f.y+k*f.x));m.lon=Math.max(G[0].lon-w*j/i.x,Math.min(G[1].lon+w*j/i.x,m.lon))}}var a={},e,d,c=R,f=R,i={x:256,y:256},m={lat:37.76487,lon:-122.41948},q=12,u=0,n=1,l=[1,18],h=0,o=1,s=0,k=1,x=0,B=-180,H=180,G=[{lat:P(B),lon:-Infinity},{lat:P(H),lon:Infinity}];a.locationCoordinate=function(j){j=r.map.locationCoordinate(j);var t=Math.pow(2,q);j.column*=t;j.row*=t;j.zoom+=q;return j};a.coordinateLocation=r.map.coordinateLocation;a.coordinatePoint=function(j,t){var w=Math.pow(2,q-t.zoom),C=Math.pow(2,q-j.zoom),
E=(t.column*w-j.column*C)*i.x*n;w=(t.row*w-j.row*C)*i.y*n;return{x:f.x+o*E-s*w,y:f.y+s*E+o*w}};a.pointCoordinate=function(j,t){var w=Math.pow(2,q-j.zoom),C=(t.x-f.x)/n,E=(t.y-f.y)/n;return{column:j.column*w+(k*C-x*E)/i.x,row:j.row*w+(x*C+k*E)/i.y,zoom:q}};a.locationPoint=function(j){var t=Math.pow(2,q+u-3)/45,w=(j.lon-m.lon)*t*i.x;j=(Q(m.lat)-Q(j.lat))*t*i.y;return{x:f.x+o*w-s*j,y:f.y+s*w+o*j}};a.pointLocation=function(j){var t=45/Math.pow(2,q+u-3),w=(j.x-f.x)*t;j=(j.y-f.y)*t;return{lon:m.lon+(k*
...
l.unapply(k);j=l.unapply(j)}x=c.locks();var y={};for(var v in x)x[v].proxyCount=0;if(i&&E>-5&&E<3){B=1<<w.zoom;if(f){$(w,t,C,0,B,s);$(C,k,w,0,B,s)}else{C=Math.floor((w.column+C.column)/2);k=Math.max(0,Math.min(B-1,Math.floor((t.row+k.row)/2)));t=Math.min(4,w.zoom);C=C>>t<<t;k=k>>t<<t;s(C,C+1,k)}}for(v in y){t=y[v];B=Math.pow(2,t.level=t.zoom-j.zoom);C=D.x*(t.column-j.column*B);k=D.y*(t.row-j.row*B);t.element.setAttribute("transform","translate("+C+","+k+")")}for(v in x)if(!(v in y)){t=c.unload(v);
t.element.parentNode.removeChild(t.element);delete t.proxyRefs}for(v in y){t=y[v];if(t.element.parentNode!=h[t.level]){h[t.level].appendChild(t.element);d.show&&d.show(t)}}}var d={},c=d.cache=r.cache(g,a).size(512),f=true,i=true,m,q,u,n,l,h;d.map=function(o){if(!arguments.length)return u;if(u){if(u==o){n.parentNode.appendChild(n);return d}u.off("move",e).off("resize",e);n.parentNode.removeChild(n);n=h=null}if(u=o){n=u.container().appendChild(r.svg("g"));q&&n.setAttribute("id",q);n.setAttribute("class",
"layer");h={};for(var s=-4;s<=-1;s++)(h[s]=n.appendChild(r.svg("g"))).setAttribute("transform","scale("+Math.pow(2,-s)+")");for(s=2;s>=1;s--)(h[s]=n.appendChild(r.svg("g"))).setAttribute("transform","scale("+Math.pow(2,-s)+")");h[0]=n.appendChild(r.svg("g"));d.init&&d.init(n);u.on("move",e).on("resize",e);e()}return d};d.container=function(){return n};d.id=function(o){if(!arguments.length)return q;q=o;return d};d.visible=function(o){if(!arguments.length)return i;i=o;u&&e();return d};d.transform=function(o){if(!arguments.length)return l;
l=o;u&&e();return d};d.zoom=function(o){if(!arguments.length)return m;m=typeof o=="function"||o==null?o:function(){return o};return d};d.tile=function(o){if(!arguments.length)return f;f=o;return d};d.dispatch=r.dispatch(d);d.on("load",function(o){if(o.tile.proxyRefs){for(var s in o.tile.proxyRefs){var k=o.tile.proxyRefs[s];--k.proxyCount<=0&&c.unload(s)&&k.element.parentNode.removeChild(k.element)}delete o.tile.proxyRefs}});return d};r.image=function(){var g=r.layer(function(e){var d=e.element=r.svg("image"),
...
a=typeof e=="string"&&/{.}/.test(e)?r.url(e):e;return g};return g};r.geoJson=function(g){function a(h,o){var s=o({lat:h[1],lon:h[0]}),k=r.svg("circle");k.setAttribute("r",4.5);k.setAttribute("cx",s.x);k.setAttribute("cy",s.y);return k}function e(h,o,s,k){k.push("M");for(var x=0;x<h.length-o;x++){p=s({lat:h[x][1],lon:h[x][0]});k.push(p.x);k.push(",");k.push(p.y);k.push("L")}k.pop()}function d(h,o,s,k){for(var x=0;x<h.length;x++)e(h[x],o,s,k);o&&k.push("Z")}function c(h,o,s,k){for(var x=[],B=0;B<o.length;B++)h(o[B],
s,k,x);if(x.length){h=r.svg("path");h.setAttribute("d",x.join(""));return h}}var f=r.layer(function(h,o){function s(x){var B=u[h.key]||(u[h.key]=[]),H=[];if(x.next)h.request=g(x.next.href,s);for(var G=0;G<x.features.length;G++){var D=x.features[G],j=D.geometry&&D.geometry.type in l&&l[D.geometry.type](D.geometry,o.locationPoint);if(j){D={element:k.appendChild(j),data:D};B.push(D);H.push(D)}}h.ready=true;f.dispatch({type:"load",tile:h,features:H})}var k=h.element=r.svg("g");o=o(h);if(n)s({features:n});
else h.request=g(i(h),s);q&&k.setAttribute("clip-path","url(#"+q+")")},function(h){h.request&&h.request.abort(true);delete u[h.key]}),i=r.url("about:blank"),m=true,q,u={},n;if(!arguments.length)g=r.queue.json;var l={Point:function(h,o){return a(h.coordinates,o)},MultiPoint:function(h,o){for(var s=r.svg("g"),k=0;k<h.coordinates.length;k++)s.appendChild(a(h.coordinates[k],o));return s},LineString:function(h,o){return c(e,[h.coordinates],0,o)},MultiLineString:function(h,o){return c(e,h.coordinates,0,
o)},Polygon:function(h,o){return c(d,[h.coordinates],1,o)},MultiPolygon:function(h,o){return c(d,h.coordinates||h.coords,1,o)},GeometryCollection:function(h,o){for(var s=r.svg("g"),k=0;k<h.geometries.length;k++){var x=h.geometries[k]&&h.geometries[k].type in l&&l[h.geometries[k].type](h.geometries[k],o);x&&s.appendChild(x)}return s}};f.url=function(h){if(!arguments.length)return i;i=typeof h=="string"?r.url(h):h;return f};f.features=function(h){if(!arguments.length)return n;h&&f.tile(false);n=h;return f};
f.clip=function(h){if(!arguments.length)return m;m=h;return f};f.init=function(h){if(m&&f.tile()){var o=f.map().tileSize(),s=h.insertBefore(r.svg("clipPath"),h.firstChild),k=s.appendChild(r.svg("rect"));s.setAttribute("id",q="org.polymaps."+r.id());k.setAttribute("width",o.x);k.setAttribute("height",o.y)}h.setAttribute("fill-rule","evenodd")};f.show=function(h){f.dispatch({type:"show",tile:h,features:u[h.key]||[]})};f.reshow=function(){var h=f.cache.locks();for(var o in h)f.show(h[o])};return f};
...
f.y=i.clientY}}var e={},d,c,f;e.map=function(i){if(!arguments.length)return d;d=i;d.container().addEventListener("mousedown",g,false);return e};window.addEventListener("mousemove",a,false);window.addEventListener("mouseup",function(i){if(f){a(i);f=null;document.body.style.cursor=c}},false);return e};r.wheel=function(){function g(){f=null}function a(m){var q=(m.wheelDelta/120||-m.detail)*0.1,u=i.mouse(m);if(Y<0&&Math.abs(m.wheelDelta)>=4800)Y=1;if(Y==1)q*=0.03;f||(f=i.pointLocation(u));i.off("move",
g);if(c)i.zoomBy(q,u,f);else if(q){var n=Date.now();if(n-d>200){i.zoomBy(q>0?+1:-1,u,f);d=n}}i.on("move",g);m.preventDefault();return false}var e={},d=0,c=true,f,i;e.smooth=function(m){if(!arguments.length)return c;c=m;return e};e.map=function(m){if(!arguments.length)return i;(i=m).on("move",g);var q=i.container();q.addEventListener("mousemove",g,false);q.addEventListener("mousewheel",a,false);q.addEventListener("DOMMouseScroll",a,false);return e};return e};var Y=/WebKit\/533/.test(navigator.userAgent)?
-1:0;r.arrow=function(){function g(l){if(!(l.ctrlKey||l.altKey||l.metaKey)){var h=Date.now(),o=0,s=0,k=0;switch(l.keyCode){case 37:if(!c.left){f=h;c.left=1;c.right||(o=u)}break;case 39:if(!c.right){f=h;c.right=1;c.left||(o=-u)}break;case 38:if(!c.up){f=h;c.up=1;c.down||(s=u)}break;case 40:if(!c.down){f=h;c.down=1;c.up||(s=-u)}break;case 109:case 189:if(!c.plus){f=h;c.plus=1;c.minus||(k=-1)}break;case 61:case 187:if(!c.minus){f=h;c.minus=1;c.plus||(k=1)}break;default:return}if(k){h=n.zoom();n.zoom(k<
0?Math.ceil(h)-1:Math.floor(h)+1)}else if(o||s)n.panBy({x:o,y:s});if(!i&&c.left|c.right|c.up|c.down)i=setInterval(e,q);l.preventDefault()}}function a(l){f=Date.now();switch(l.keyCode){case 37:c.left=0;break;case 39:c.right=0;break;case 38:c.up=0;break;case 40:c.down=0;break;case 109:case 189:c.plus=0;break;case 61:case 187:c.minus=0;break;default:return}if(i&&!(c.left|c.right|c.up|c.down))i=clearInterval(i);l.preventDefault()}function e(){if(n)if(!(Date.now()<f+m)){var l=(c.left-c.right)*u,h=(c.up-
c.down)*u;if(l||h)n.panBy({x:l,y:h})}}var d={},c={left:0,right:0,up:0,down:0,plus:0,minus:0},f=0,i,m=250,q=50,u=16,n;d.map=function(l){if(!arguments.length)return n;n=l;var h=n.focusableParent();h.addEventListener("keydown",g,false);h.addEventListener("keyup",a,false);return d};d.speed=function(l){if(!arguments.length)return u;u=l;return d};return d};r.hash=function(){function g(){var i=f.center(),m=f.zoom(),q=Math.max(0,Math.ceil(Math.log(m)/Math.LN2));i="#"+m.toFixed(2)+"/"+i.lat.toFixed(q)+"/"+
i.lon.toFixed(q);if(d!==i)location.replace(d=i)}function a(){if(location.hash!==d){var i=(d=location.hash).substring(1).split("/").map(Number);if(i.length<3||i.some(isNaN))g();else{var m=f.size();f.zoomBy(i[0]-f.zoom(),{x:m.x/2,y:m.y/2},{lat:Math.min(c,Math.max(-c,i[1])),lon:i[2]})}}}var e={},d,c=89.99999999,f;e.map=function(i){if(!arguments.length)return f;f&&f.off("move",g);(f=i).on("move",g);window.addEventListener("hashchange",a,false);location.hash?a():g();return e};return e};r.interact=function(){var g=
{},a=r.drag(),e=r.wheel(),d=r.dblclick(),c=r.arrow();g.map=function(f){a.map(f);e.map(f);d.map(f);c.map(f);return g};return g};r.compass=function(){function g(y){o.setAttribute("class","compass active");w||(w=setInterval(function(){C&&Date.now()>B+H&&E.panBy(C)},G));C&&E.panBy(C);B=Date.now();m(y)}function a(){o.setAttribute("class","compass");if(w){clearInterval(w);w=0}}function e(y){return function(){y?this.setAttribute("class","active"):this.removeAttribute("class");C=y}}function d(y){return function(v){o.setAttribute("class",
"compass active");var z=E.zoom();E.zoom(y<0?Math.ceil(z)-1:Math.floor(z)+1);m(v)}}function c(y){return function(v){E.zoom(y);m(v)}}function f(){this.setAttribute("class","active")}function i(){this.removeAttribute("class")}function m(y){y.stopPropagation();y.preventDefault()}function q(y){var v=Math.SQRT1_2*k,z=k*0.7,A=k*0.2,F=r.svg("g"),I=F.appendChild(r.svg("path")),N=F.appendChild(r.svg("path"));I.setAttribute("class","direction");I.setAttribute("pointer-events","all");I.setAttribute("d","M0,0L"+
v+","+v+"A"+k+","+k+" 0 0,1 "+-v+","+v+"Z");N.setAttribute("class","chevron");N.setAttribute("d","M"+A+","+(z-A)+"L0,"+z+" "+-A+","+(z-A));N.setAttribute("pointer-events","none");F.addEventListener("mousedown",g,false);F.addEventListener("mouseover",e(y),false);F.addEventListener("mouseout",e(null),false);F.addEventListener("dblclick",m,false);return F}function u(y){var v=k*0.4,z=v/2,A=r.svg("g"),F=A.appendChild(r.svg("path")),I=A.appendChild(r.svg("path")),N=A.appendChild(r.svg("path")),M=A.appendChild(r.svg("path"));
...
return h};r.grid=function(){function g(){var c=d.firstChild,f=e.size(),i=e.pointLocation(R);e.pointLocation(f);var m=Math.pow(2,4-Math.round(e.zoom()));i.lat=Math.floor(i.lat/m)*m;i.lon=Math.ceil(i.lon/m)*m;for(var q;(q=e.locationPoint(i).x)<=f.x;i.lon+=m){c||(c=d.appendChild(r.svg("line")));c.setAttribute("x1",q);c.setAttribute("x2",q);c.setAttribute("y1",0);c.setAttribute("y2",f.y);c=c.nextSibling}for(;(q=e.locationPoint(i).y)<=f.y;i.lat-=m){c||(c=d.appendChild(r.svg("line")));c.setAttribute("y1",
q);c.setAttribute("y2",q);c.setAttribute("x1",0);c.setAttribute("x2",f.x);c=c.nextSibling}for(;c;){f=c.nextSibling;d.removeChild(c);c=f}}var a={},e,d;a.map=function(c){if(!arguments.length)return e;e=c;e.on("move",g);e.on("resize",g);if(!d){d=r.svg("g");d.setAttribute("class","grid")}e.container().appendChild(d);g();return a};return a}})(org.polymaps);
