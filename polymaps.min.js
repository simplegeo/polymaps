...
false);return c};t.touch=function(){function d(i){f&&c();i.preventDefault();f=l(i)}function g(i){if(!f)return d(i);i.preventDefault();i=l(i);var n={x:i.clientX-f.clientX,y:i.clientY-f.clientY,scale:i.scale-f.scale};if(n.x||n.y||n.scale){var o=a.mouse(f);o=a.pointLocation(o);a.zoomBy(n.scale,a.mouse(i),o)}f=i}function c(){f=null}function l(i){for(var n=i.touches,o=0,m=0,j=0;j<n.length;++j){var u=n[j];o+=u.clientX;m+=u.clientY}return{clientX:o/n.length,clientY:m/n.length,scale:i.scale}}var h={},a,f;
h.map=function(i){if(!arguments.length)return a;a=i;var n=a.container();n.addEventListener("touchstart",d,false);n.addEventListener("touchmove",g,false);n.addEventListener("touchend",c,false);n.addEventListener("touchcancel",c,false);return h};return h};t.wheel=function(){function d(){i=null}function g(m){var j=(m.wheelDelta/120||-m.detail)*0.1,u;if(ba<0){u=Date.now();var y=u-h;if(y>9&&Math.abs(m.wheelDelta)/y>=50)ba=1;h=u}if(ba==1)j*=0.03;if(!a&&j){u=Date.now();if(u-l>200){j=j>0?+1:-1;l=u}else j=
0}if(j)switch(f){case "mouse":u=n.mouse(m);i||(i=n.pointLocation(u));n.off("move",d).zoomBy(j,u,i).on("move",d);break;case "location":n.zoomBy(j,n.locationPoint(i),i);break;default:n.zoomBy(j);break}m.preventDefault();return false}var c={},l=0,h=0,a=true,f="mouse",i,n,o;c.smooth=function(m){if(!arguments.length)return a;a=m;return c};c.zoom=function(m,j){if(!arguments.length)return f;f=m;i=j;if(n)f=="mouse"?n.on("move",d):n.off("move",d);return c};c.map=function(m){if(!arguments.length)return n;if(n){o.removeEventListener("mousemove",
d,false);o.removeEventListener("mousewheel",g,false);o.removeEventListener("DOMMouseScroll",g,false);o=null;n.off("move",d)}if(n=m){f=="mouse"&&n.on("move",d);o=n.container();o.addEventListener("mousemove",d,false);o.addEventListener("mousewheel",g,false);o.addEventListener("DOMMouseScroll",g,false)}return c};return c};var ba=/WebKit\/533/.test(navigator.userAgent)?-1:0;t.arrow=function(){function d(y){if(!(y.ctrlKey||y.altKey||y.metaKey)){var A=Date.now(),C=0,E=0;switch(y.keyCode){case 37:if(!a.left){f=
A;a.left=1;a.right||(C=m)}break;case 39:if(!a.right){f=A;a.right=1;a.left||(C=-m)}break;case 38:if(!a.up){f=A;a.up=1;a.down||(E=m)}break;case 40:if(!a.down){f=A;a.down=1;a.up||(E=-m)}break;default:return}if(C||E)j.panBy({x:C,y:E});if(!i&&a.left|a.right|a.up|a.down)i=setInterval(l,o);y.preventDefault()}}function g(y){f=Date.now();switch(y.keyCode){case 37:a.left=0;break;case 39:a.right=0;break;case 38:a.up=0;break;case 40:a.down=0;break;default:return}if(i&&!(a.left|a.right|a.up|a.down))i=clearInterval(i);
y.preventDefault()}function c(y){switch(y.charCode){case 45:case 95:j.zoom(Math.ceil(j.zoom())-1);break;case 43:case 61:j.zoom(Math.floor(j.zoom())+1);break;default:return}y.preventDefault()}function l(){if(j)if(!(Date.now()<f+n)){var y=(a.left-a.right)*m,A=(a.up-a.down)*m;if(y||A)j.panBy({x:y,y:A})}}var h={},a={left:0,right:0,up:0,down:0},f=0,i,n=250,o=50,m=16,j,u;h.map=function(y){if(!arguments.length)return j;if(j){u.removeEventListener("keypress",c,false);u.removeEventListener("keydown",d,false);
u.removeEventListener("keyup",g,false);u=null}if(j=y){u=j.focusableParent();u.addEventListener("keypress",c,false);u.addEventListener("keydown",d,false);u.addEventListener("keyup",g,false)}return h};h.speed=function(y){if(!arguments.length)return m;m=y;return h};return h};t.hash=function(){function d(){var i=f(h);if(l!==i)location.replace(l=i)}function g(){if(location.hash!==l)if(a(h,(l=location.hash).substring(1)))d()}var c={},l,h,a=function(i,n){var o=n.split("/").map(Number);if(o.length<3||o.some(isNaN))return true;
else{var m=i.size();i.zoomBy(o[0]-i.zoom(),{x:m.x/2,y:m.y/2},{lat:Math.min(89.99999999,Math.max(-89.99999999,o[1])),lon:o[2]})}},f=function(i){var n=i.center();i=i.zoom();var o=Math.max(0,Math.ceil(Math.log(i)/Math.LN2));return"#"+i.toFixed(2)+"/"+n.lat.toFixed(o)+"/"+n.lon.toFixed(o)};c.map=function(i){if(!arguments.length)return h;if(h){h.off("move",d);window.removeEventListener("hashchange",g,false)}if(h=i){h.on("move",d);window.addEventListener("hashchange",g,false);location.hash?g():d()}return c};
c.parser=function(i){if(!arguments.length)return a;a=i;return c};c.formatter=function(i){if(!arguments.length)return f;f=i;return c};return c};t.interact=function(){var d={},g=t.drag(),c=t.wheel(),l=t.dblclick(),h=t.arrow();d.map=function(a){g.map(a);c.map(a);l.map(a);h.map(a);return d};return d};t.compass=function(){function d(x){B.setAttribute("class","compass active");I||(I=setInterval(g,s));R&&H.panBy(R);w=Date.now();return m(x)}function g(){R&&Date.now()>w+e&&H.panBy(R)}function c(x){if(x.shiftKey){F=
{x0:H.mouse(x)};H.focusableParent().focus();return m(x)}}function l(x){if(F){F.x1=H.mouse(x);O.setAttribute("x",Math.min(F.x0.x,F.x1.x));O.setAttribute("y",Math.min(F.x0.y,F.x1.y));O.setAttribute("width",Math.abs(F.x0.x-F.x1.x));O.setAttribute("height",Math.abs(F.x0.y-F.x1.y));O.removeAttribute("display")}}function h(){B.setAttribute("class","compass");if(F){if(F.x1){H.extent([H.pointLocation({x:Math.min(F.x0.x,F.x1.x),y:Math.max(F.x0.y,F.x1.y)}),H.pointLocation({x:Math.max(F.x0.x,F.x1.x),y:Math.min(F.x0.y,
F.x1.y)})]);O.setAttribute("display","none")}F=null}if(I){clearInterval(I);I=0}}function a(x){return function(){x?this.setAttribute("class","active"):this.removeAttribute("class");R=x}}function f(x){return function(D){B.setAttribute("class","compass active");var G=H.zoom();H.zoom(x<0?Math.ceil(G)-1:Math.floor(G)+1);return m(D)}}function i(x){return function(D){H.zoom(x);return m(D)}}function n(){this.setAttribute("class","active")}function o(){this.removeAttribute("class")}function m(x){x.stopPropagation();
x.preventDefault();return false}function j(x){var D=Math.SQRT1_2*k,G=k*0.7,J=k*0.2,N=t.svg("g"),P=N.appendChild(t.svg("path")),Q=N.appendChild(t.svg("path"));P.setAttribute("class","direction");P.setAttribute("pointer-events","all");P.setAttribute("d","M0,0L"+D+","+D+"A"+k+","+k+" 0 0,1 "+-D+","+D+"Z");Q.setAttribute("class","chevron");Q.setAttribute("d","M"+J+","+(G-J)+"L0,"+G+" "+-J+","+(G-J));Q.setAttribute("pointer-events","none");N.addEventListener("mousedown",d,false);N.addEventListener("mouseover",
a(x),false);N.addEventListener("mouseout",a(null),false);N.addEventListener("dblclick",m,false);return N}function u(x){var D=k*0.4,G=D/2,J=t.svg("g"),N=J.appendChild(t.svg("path")),P=J.appendChild(t.svg("path")),Q=J.appendChild(t.svg("path")),X=J.appendChild(t.svg("path"));N.setAttribute("class","back");N.setAttribute("d","M"+-D+",0V"+-D+"A"+D+","+D+" 0 1,1 "+D+","+-D+"V0Z");P.setAttribute("class","direction");P.setAttribute("d",N.getAttribute("d"));Q.setAttribute("class","chevron");Q.setAttribute("d",
"M"+-G+","+-D+"H"+G+(x>0?"M0,"+(-D-G)+"V"+-G:""));X.setAttribute("class","fore");X.setAttribute("fill","none");X.setAttribute("d",N.getAttribute("d"));J.addEventListener("mousedown",f(x),false);J.addEventListener("mouseover",n,false);J.addEventListener("mouseout",o,false);J.addEventListener("dblclick",m,false);return J}function y(x){var D=k*0.2,G=k*0.4,J=t.svg("g"),N=J.appendChild(t.svg("rect")),P=J.appendChild(t.svg("path"));N.setAttribute("pointer-events","all");N.setAttribute("fill","none");N.setAttribute("x",
-G);N.setAttribute("y",-0.75*G);N.setAttribute("width",2*G);N.setAttribute("height",1.5*G);P.setAttribute("class","chevron");P.setAttribute("d","M"+-D+",0H"+D);J.addEventListener("mousedown",i(x),false);J.addEventListener("dblclick",m,false);return J}function A(){var x=k+6,D=x,G=H.size();switch(r){case "top-left":break;case "top-right":x=G.x-x;break;case "bottom-left":D=G.y-D;break;case "bottom-right":x=G.x-x;D=G.y-D;break}x="translate("+x+","+D+")";M&&M.setAttribute("transform",x);v&&v.setAttribute("transform",
x);for(var J in K)J==H.zoom()?K[J].setAttribute("class","active"):K[J].removeAttribute("class")}function C(){for(;B.lastChild;)B.removeChild(B.lastChild);B.appendChild(O);if(L!="none"){M=B.appendChild(t.svg("g"));M.setAttribute("class","pan");var x=M.appendChild(t.svg("circle"));x.setAttribute("class","back");x.setAttribute("r",k);M.appendChild(j({x:0,y:-q})).setAttribute("transform","rotate(0)");M.appendChild(j({x:q,y:0})).setAttribute("transform","rotate(90)");M.appendChild(j({x:0,y:q})).setAttribute("transform",
"rotate(180)");M.appendChild(j({x:-q,y:0})).setAttribute("transform","rotate(270)");x=M.appendChild(t.svg("circle"));x.setAttribute("fill","none");x.setAttribute("class","fore");x.setAttribute("r",k)}else M=null;if(z!="none"){v=B.appendChild(t.svg("g"));v.setAttribute("class","zoom");x=-0.5;if(z=="big"){K={};var D=H.zoomRange()[0];for(x=0;D<=H.zoomRange()[1];D++,x++)(K[D]=v.appendChild(y(D))).setAttribute("transform","translate(0,"+-(x+0.75)*k*0.4+")")}D=L=="none"?0.4:2;v.setAttribute("transform",
"translate(0,"+k*(/^top-/.test(r)?D+(x+0.5)*0.4:-D)+")");v.appendChild(u(+1)).setAttribute("transform","translate(0,"+-(x+0.5)*k*0.4+")");v.appendChild(u(-1)).setAttribute("transform","scale(-1)")}else v=null;A()}var E={},B=t.svg("g"),K={},k=30,q=16,w=0,e=250,s=50,r="top-left",z="small",v,L="small",I,R,M,F,O=t.svg("rect"),H,T,U;B.setAttribute("class","compass");O.setAttribute("class","back fore");O.setAttribute("pointer-events","none");O.setAttribute("display","none");E.radius=function(x){if(!arguments.length)return k;
k=x;H&&C();return E};E.speed=function(x){if(!arguments.length)return k;q=x;return E};E.position=function(x){if(!arguments.length)return r;r=x;H&&C();return E};E.pan=function(x){if(!arguments.length)return L;L=x;H&&C();return E};E.zoom=function(x){if(!arguments.length)return z;z=x;H&&C();return E};E.map=function(x){if(!arguments.length)return H;if(H){T.removeEventListener("mousedown",c,false);T.removeChild(B);T=null;U.removeEventListener("mousemove",l,false);U.removeEventListener("mouseup",h,false);
U=null;H.off("move",A).off("resize",A)}if(H=x){T=H.container();T.appendChild(B);T.addEventListener("mousedown",c,false);U=T.ownerDocument.defaultView;U.addEventListener("mousemove",l,false);U.addEventListener("mouseup",h,false);H.on("move",A).on("resize",A);C()}return E};return E};t.grid=function(){function d(){var h=l.firstChild,a=c.size(),f=c.pointLocation(Y);c.pointLocation(a);var i=Math.pow(2,4-Math.round(c.zoom()));f.lat=Math.floor(f.lat/i)*i;f.lon=Math.ceil(f.lon/i)*i;for(var n;(n=c.locationPoint(f).x)<=
a.x;f.lon+=i){h||(h=l.appendChild(t.svg("line")));h.setAttribute("x1",n);h.setAttribute("x2",n);h.setAttribute("y1",0);h.setAttribute("y2",a.y);h=h.nextSibling}for(;(n=c.locationPoint(f).y)<=a.y;f.lat-=i){h||(h=l.appendChild(t.svg("line")));h.setAttribute("y1",n);h.setAttribute("y2",n);h.setAttribute("x1",0);h.setAttribute("x2",a.x);h=h.nextSibling}for(;h;){a=h.nextSibling;l.removeChild(h);h=a}}var g={},c,l=t.svg("g");l.setAttribute("class","grid");g.map=function(h){if(!arguments.length)return c;
if(c){l.parentNode.removeChild(l);c.off("move",d).off("resize",d)}if(c=h){c.on("move",d).on("resize",d);c.container().appendChild(l);c.dispatch({type:"move"})}return g};return g};t.stylist=function(){function d(h){var a=h.features.length,f=g.length,i=c.length,n,o,m,j,u,y;for(u=0;u<a;++u)if(o=(n=h.features[u]).element){n=n.data;for(y=0;y<f;++y){j=(m=g[y]).value;if(typeof j==="function")j=j.call(null,n);j==null?m.name.local?o.removeAttributeNS(m.name.space,m.name.local):o.removeAttribute(m.name):m.name.local?
o.setAttributeNS(m.name.space,m.name.local,j):o.setAttribute(m.name,j)}for(y=0;y<i;++y){j=(m=c[y]).value;if(typeof j==="function")j=j.call(null,n);j==null?o.style.removeProperty(m.name):o.style.setProperty(m.name,j,m.priority)}if(j=l){if(typeof j==="function")j=j.call(null,n);for(;o.lastChild;)o.removeChild(o.lastChild);j!=null&&o.appendChild(t.svg("title")).appendChild(document.createTextNode(j))}}}var g=[],c=[],l;d.attr=function(h,a){g.push({name:fa(h),value:a});return d};d.style=function(h,a,f){c.push({name:h,
value:a,priority:arguments.length<3?null:f});return d};d.title=function(h){l=h;return d};return d}})(org.polymaps);
