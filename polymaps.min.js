...
"translate("+-y+","+-D+")");for(var J in K)J==H.zoom()?K[J].setAttribute("class","active"):K[J].removeAttribute("class")}function C(){for(;B.lastChild;)B.removeChild(B.lastChild);B.appendChild(N);if(L!="none"){O=B.appendChild(r.svg("g"));O.setAttribute("class","pan");var y=O.appendChild(r.svg("circle"));y.setAttribute("class","back");y.setAttribute("r",j);O.appendChild(i({x:0,y:-q})).setAttribute("transform","rotate(0)");O.appendChild(i({x:q,y:0})).setAttribute("transform","rotate(90)");O.appendChild(i({x:0,
y:q})).setAttribute("transform","rotate(180)");O.appendChild(i({x:-q,y:0})).setAttribute("transform","rotate(270)");y=O.appendChild(r.svg("circle"));y.setAttribute("fill","none");y.setAttribute("class","fore");y.setAttribute("r",j)}else O=null;if(z!="none"){v=B.appendChild(r.svg("g"));v.setAttribute("class","zoom");y=-0.5;if(z=="big"){K={};var D=H.zoomRange()[0];for(y=0;D<=H.zoomRange()[1];D++,y++)(K[D]=v.appendChild(x(D))).setAttribute("transform","translate(0,"+-(y+0.75)*j*0.4+")")}D=L=="none"?
0.4:2;v.setAttribute("transform","translate(0,"+j*(/^top-/.test(s)?D+(y+0.5)*0.4:-D)+")");v.appendChild(u(+1)).setAttribute("transform","translate(0,"+-(y+0.5)*j*0.4+")");v.appendChild(u(-1)).setAttribute("transform","scale(-1)")}else v=null;A()}var E={},B=r.svg("g"),K={},j=30,q=16,w=0,g=250,t=50,s="top-left",z="small",v,L="small",I,R,O,F,N=r.svg("rect"),H,T,U;B.setAttribute("class","compass");N.setAttribute("class","back fore");N.setAttribute("pointer-events","none");N.setAttribute("display","none");
E.radius=function(y){if(!arguments.length)return j;j=y;H&&C();return E};E.speed=function(y){if(!arguments.length)return j;q=y;return E};E.position=function(y){if(!arguments.length)return s;s=y;H&&C();return E};E.pan=function(y){if(!arguments.length)return L;L=y;H&&C();return E};E.zoom=function(y){if(!arguments.length)return z;z=y;H&&C();return E};E.map=function(y){if(!arguments.length)return H;if(H){T.removeEventListener("mousedown",c,false);T.removeChild(B);T=null;U.removeEventListener("mousemove",
l,false);U.removeEventListener("mouseup",f,false);U=null;H.off("move",A).off("resize",A)}if(H=y){T=H.container();T.appendChild(B);T.addEventListener("mousedown",c,false);U=T.ownerDocument.defaultView;U.addEventListener("mousemove",l,false);U.addEventListener("mouseup",f,false);H.on("move",A).on("resize",A);C()}return E};return E};r.grid=function(){function e(){var f=l.firstChild,a=c.size(),d=c.pointLocation(Y);c.pointLocation(a);var k=Math.pow(2,4-Math.round(c.zoom()));d.lat=Math.floor(d.lat/k)*k;
d.lon=Math.ceil(d.lon/k)*k;for(var n;(n=c.locationPoint(d).x)<=a.x;d.lon+=k){f||(f=l.appendChild(r.svg("line")));f.setAttribute("x1",n);f.setAttribute("x2",n);f.setAttribute("y1",0);f.setAttribute("y2",a.y);f=f.nextSibling}for(;(n=c.locationPoint(d).y)<=a.y;d.lat-=k){f||(f=l.appendChild(r.svg("line")));f.setAttribute("y1",n);f.setAttribute("y2",n);f.setAttribute("x1",0);f.setAttribute("x2",a.x);f=f.nextSibling}for(;f;){a=f.nextSibling;l.removeChild(f);f=a}}var h={},c,l=r.svg("g");l.setAttribute("class",
"grid");h.map=function(f){if(!arguments.length)return c;if(c){l.parentNode.removeChild(l);c.off("move",e).off("resize",e)}if(c=f){c.on("move",e).on("resize",e);c.container().appendChild(l);c.dispatch({type:"move"})}return h};return h};r.stylist=function(){function e(f){var a=f.features.length,d=h.length,k=c.length,n,o,m,i,u,x;for(u=0;u<a;++u)if(o=(n=f.features[u]).element){n=n.data;for(x=0;x<d;++x){i=(m=h[x]).value;if(typeof i==="function")i=i.call(null,n);i==null?m.name.local?o.removeAttributeNS(m.name.space,
m.name.local):o.removeAttribute(m.name):m.name.local?o.setAttributeNS(m.name.space,m.name.local,i):o.setAttribute(m.name,i)}for(x=0;x<k;++x){i=(m=c[x]).value;if(typeof i==="function")i=i.call(null,n);i==null?o.style.removeProperty(m.name):o.style.setProperty(m.name,i,m.priority)}if(i=l){if(typeof i==="function")i=i.call(null,n);for(;o.lastChild;)o.removeChild(o.lastChild);i!=null&&o.appendChild(r.svg("title")).appendChild(document.createTextNode(i))}}}var h=[],c=[],l;e.attr=function(f,a){h.push({name:fa(f),
value:a});return e};e.style=function(f,a,d){c.push({name:f,value:a,priority:arguments.length<3?null:d});return e};e.title=function(f){l=f;return e};return e}})(org.polymaps);
